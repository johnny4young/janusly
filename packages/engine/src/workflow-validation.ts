...
export function validateWorkflow(workflow: any): WorkflowValidationResult {
  ...
  if (!Array.isArray(workflow.nodes)) {
    issues.push({ code: "missing_nodes", message: "Workflow must include a nodes array" });
  }

  if (!Array.isArray(workflow.edges)) {
    issues.push({ code: "missing_edges", message: "Workflow must include an edges array" });
  }
  ...
  for (const node of nodes) {
    ...
    if (!supportedNodeTypes.has(node.type)) {
      issues.push({ code: "unsupported_node_type", message: `Unsupported node type: ${node.type}`, nodeId: node.id });
    }

    if (node.type === "http" && !node.config?.url) {
      issues.push({ code: "http_missing_url", message: "HTTP node requires config.url", nodeId: node.id });
    }

    if (node.type === "tool" && !node.config?.tool) {
      issues.push({ code: "tool_missing_name", message: "Tool node requires config.tool", nodeId: node.id });
    }
  ...
  for (const [index, edge] of edges.entries()) {
    ...
    if (!nodeIds.has(edge.from)) {
      issues.push({ code: "edge_invalid_from", message: `Edge source does not exist: ${edge.from}`, edgeId });
    }

    if (!nodeIds.has(edge.to)) {
      issues.push({ code: "edge_invalid_to", message: `Edge target does not exist: ${edge.to}`, edgeId });
    }
  }

  if (hasCycle(nodes, edges)) {
    issues.push({ code: "cycle_detected", message: "Workflow graph contains a cycle" });
  }
  ...
  if (nodes.length > 0 && startNodes.length === 0) {
    issues.push({ code: "missing_start_node", message: "Workflow must have at least one start node" });
  }
...
function hasCycle(nodes: any[], edges: any[]) {
  ...
  for (const edge of edges) {
    if (!graph.has(edge.from)) continue;

...
describe("executeNode run-input plumbing", () => {
  beforeEach(() => {
...
describe("executeNode timeout enforcement", () => {
  beforeEach(() => {
  ...
  it("enforces config.timeoutMs at the chokepoint — a hung executor throws NODE_TIMEOUT (freeing the worker)", async () => {
    // Executor that never resolves — without the timeout this blocked the worker until

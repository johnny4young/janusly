...
vi.mock("@janusly/db", () => ({
  ...
  workflowMetadata: { tags: "tags", orgId: "org_id", workflowId: "workflow_id" },
}));
...
describe("listWorkflowsWithRunSummary", () => {
  ...
  it("folds the run aggregate onto each workflow; zero-run rows default to 0 / null", async () => {
    baseRows = [
      { id: "wf-a", orgId: "org-1", name: "A", createdBy: "u", createdAt: new Date("2026-01-01T00:00:00Z"), status: "paused_upstream_degraded", pausedReason: "Upstream \"stripe\" degraded", tags: [] },
      { id: "wf-b", orgId: "org-1", name: "B", createdBy: null, createdAt: new Date("2026-01-02T00:00:00Z"), status: "active", pausedReason: null, tags: [] },
    ];
    ...
    expect(rows).toEqual([
      { id: "wf-a", orgId: "org-1", name: "A", createdBy: "u", createdAt: new Date("2026-01-01T00:00:00Z"), lastRunStatus: "failed", runCount: 3, status: "paused_upstream_degraded", pausedReason: "Upstream \"stripe\" degraded", tags: [] },
      { id: "wf-b", orgId: "org-1", name: "B", createdBy: null, createdAt: new Date("2026-01-02T00:00:00Z"), lastRunStatus: null, runCount: 0, status: "active", pausedReason: null, tags: [] },
    ]);
  });

  it("folds workflow_metadata.tags onto each row (null → []) and exercises the tag filter branch", async () => {
    baseRows = [
      { id: "wf-a", orgId: "org-1", name: "A", createdBy: null, createdAt: new Date("2026-01-01T00:00:00Z"), status: "active", pausedReason: null, tags: ["billing", "urgent"] },
      { id: "wf-b", orgId: "org-1", name: "B", createdBy: null, createdAt: new Date("2026-01-02T00:00:00Z"), status: "active", pausedReason: null, tags: null },
    ];
    aggRows = [];
    ...
    // mapping (the real filtering is smoke-verified against Postgres).

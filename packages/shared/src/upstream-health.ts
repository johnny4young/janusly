 ...
 * background poller in `packages/engine/src/upstream-health-poller.ts` fetches
 ...
 *
 * Used by:
 *  - `packages/data/src/upstreamHealthSourcesRepo.ts` (CRUD + derived status)
 *  - `packages/engine/src/upstream-health-poller.ts` (poll + parse + evaluate)

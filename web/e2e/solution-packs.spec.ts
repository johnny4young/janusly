...
test('Solution Packs install, sample-run, and recovery-drill flows work from the UI', async ({ page, request }) => {
  ...
  await expect(activityRecovery).toHaveAttribute('data-dead-letter-id', drillBody.deadLetterId)
  await expect(activityRecovery).toContainText('Incident triage')
  await expect(activityRecovery).toContainText('Run a tool')
  await expect(activityRecovery).toContainText('worker_stalled')
  const drillContext = activityRecovery.getByTestId('dlq-recovery-drill-context')
  await expect(drillContext).toContainText('Recovery drill')
  await expect(drillContext).toContainText('Real reaper path')
  await dismissToasts(page)
  await captureEvidence(activityRecovery, 'solution-packs-en-worker-interruption-recovery-detail')
...
test('Spanish recovery drills remain usable on mobile without horizontal overflow', async ({ page }) => {
  ...
  const drillBody = await drillResponse.json() as { deadLetterId: string }
  await expect(page.getByText('Ejercicio de recuperación creado')).toBeVisible()
  const activityRecovery = page.getByTestId('activity-recovery-detail')
  await expect(activityRecovery).toBeVisible({ timeout: 30_000 })
  await expect(activityRecovery).toHaveAttribute('data-dead-letter-id', drillBody.deadLetterId)
  await expect(activityRecovery).toContainText('Ejecutar herramienta')
  await expect(activityRecovery).toContainText('JANUSLY_RUNTIME_DRILL_MISSING_SECRET')
  const drillContext = activityRecovery.getByTestId('dlq-recovery-drill-context')
  ...
  await captureEvidence(activityRecovery, 'solution-packs-es-runtime-failure-detail')

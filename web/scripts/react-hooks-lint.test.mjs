...
test('the repository lint configuration rejects a missing effect dependency', () => {
  const result = spawnSync(oxlint, ['--format', 'json', 'scripts/fixtures/react-exhaustive-deps-negative.tsx'], {
    cwd: webRoot,
    encoding: 'utf8',
  })
  ...
  const report = JSON.parse(result.stdout)
  assert.ok(
    report.diagnostics.some(({ code }) => code === 'react-hooks(exhaustive-deps)'),
    `expected react/exhaustive-deps, received ${result.stdout}`,
  )
})

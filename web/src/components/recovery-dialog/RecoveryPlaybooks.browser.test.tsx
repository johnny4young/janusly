...
it('renders the scorecard rate in a real browser once the sample floor is met', async () => {
  render(<PlaybookMatchCard playbook={{ ...playbook, successfulUses: 20, regressions: 3 }} busy={null} onUse={vi.fn()} onRetire={vi.fn()} />)

  const rate = await screen.findByTestId('playbook-scorecard-rate')
  expect(rate).toBeVisible()
  expect(rate.textContent).toContain('87')
})

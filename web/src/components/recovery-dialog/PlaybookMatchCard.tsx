...
export function PlaybookMatchCard({
  ...
  const scorecard = resolvePlaybookScorecard(playbook.successfulUses, playbook.regressions)
  return (
    <section className="we-recovery-playbook" data-testid="recovery-playbook-match" aria-labelledby="recovery-playbook-match-title">
      ...
      <dl className="we-recovery-playbook__facts">
        {scorecard.ratePercent !== null && (
          <div data-testid="playbook-scorecard-rate">
            <dt>{t('recoveryDialog.playbook.successRate')}</dt>
            <dd>{t('recoveryDialog.playbook.successRateValue', { percent: scorecard.ratePercent, total: scorecard.total })}</dd>
          </div>
        )}
        <div><dt>{t('recoveryDialog.playbook.successfulUses')}</dt><dd>{playbook.successfulUses}</dd></div>
        {playbook.regressions > 0 && (
          <div data-testid="playbook-scorecard-regressions">
            <dt>{t('recoveryDialog.playbook.regressions')}</dt>
            <dd>{playbook.regressions}</dd>
          </div>
        )}

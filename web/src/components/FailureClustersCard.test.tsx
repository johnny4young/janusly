...
vi.mock('../api', async () => {
  ...
  return {
    ...
    contractApi: contractApiOver(api, {
      ...
      'GET /dlq/clusters': (value: { clusters?: Record<string, unknown>[] }) => ({
        ...value,
        clusters: value.clusters?.map(cluster => ({ recurredAfterRecovery: false, ...cluster })),
      }),
      'GET /workflows/health/delta': healthDelta,
      'POST /ai/patch-workflow': (value: Record<string, unknown>) => patchResponse(value, 'upstream 502'),
      'POST /recovery/playbooks/{id}/use': (value: { suggestion: Record<string, unknown> }) => ({ suggestion: patchResponse(value.suggestion, 'upstream 502') }),
    }),
  }
})
...
describe('<FailureClustersCard />', () => {
  ...
  it('keeps the cluster success state mounted while refreshed data arrives', async () => {
    ...
    const clustersPayload = { clusters: [cluster], totalSamples: 2, windowDays: 30 }
    const refreshedClustersPayload = { clusters: [], totalSamples: 0, windowDays: 30 }
    let clusterRequestCount = 0
    let resolveClusterRefresh: ((payload: typeof clustersPayload) => void) | undefined

    vi.mocked(api).mockImplementation(async (path: string) => {
      if (path === '/dlq/clusters') {
        clusterRequestCount += 1
        if (clusterRequestCount === 1) return clustersPayload
        return new Promise<typeof clustersPayload>((resolve) => {
    ...
    resolveRefresh(refreshedClustersPayload)

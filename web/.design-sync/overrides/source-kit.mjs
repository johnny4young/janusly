// forked from design-sync lib/source-kit.mjs - keep app entry modules (src/main.tsx) out of the synthesized bundle entry
...
// Janusly fork: modules that are application entry points, not components.
// `src/main.tsx` boots the real app at import time — it calls bootTheme(),
// bootstrapI18n() and createRoot().render() — and it imports `src/index.css`,
// which pulls RAW `tailwindcss/theme.css` + `preflight.css` into the bundle.
// esbuild cannot evaluate Tailwind's compile-time `--theme()` function, so the
// emitted CSS carried `font-family: --theme(--default-font-family, …)`, an
// invalid declaration browsers drop — leaving every preview card in the
// browser default serif. Keeping this file out of the entry fixes both.
const APP_ENTRY_RX = /(^|\/)main\.(tsx|jsx)$/;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !APP_ENTRY_RX.test(slash(p)));

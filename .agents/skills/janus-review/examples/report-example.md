...
This is what a complete review report looks like for the Drizzle migrations work shipped via `janus-ship` in `.agents/skills/janus-ship/examples/report-example.md`. The reviewer found two real bugs (one in the implementer's diff, one collateral) and one design concern.

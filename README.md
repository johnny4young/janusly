...
- **Reproducible recovery drills.** Solution Packs expose safe, selectable credential, AI-output, rate-limit, contract-drift, and upstream-failure scenarios. Each deterministic drill records its source and then enters the same recovery queue used by runtime failures; worker-interruption recovery remains a real stalled-node-reaper path rather than a synthetic claim.

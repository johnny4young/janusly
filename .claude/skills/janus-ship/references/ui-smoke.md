...
This protocol is mandatory whenever the smoke is for a UI change. It is the
single source of truth for both `janus-ship` (validating the implementer's UI)
and `janus-review` (validating the reviewer's own inline UI fixes).

## When a UI smoke is required

- **janus-ship:** the diff adds or changes ANY `apps/web/**` surface a user sees
  — a component, panel, button, chip, modal, toast, overlay, layout, or copy.
- **janus-review:** the reviewer's OWN inline fix changed a web surface, OR the
  staged diff is UI-heavy and the implementer left no evidence / only a generic
  full-page frame.
...
- **Inverse case:** when the ticket's OWN surface IS a modal/overlay/drawer, open
  it and capture it open — and hide everything else competing for the frame.

### 3. Capture the states the ticket introduces — not one screen
- A UI change has states. Capture each meaningful one the ticket adds or alters,
  one PNG per state:
...
- Before each screenshot, assert via `browser_snapshot` or a `browser_evaluate`
  DOM probe that the ticket's element is **present, visible, in the expected
...
- Artifacts go under `output/janus-ship/eng-NNN/` (ship) or
  `output/review/eng-NNN/` (review). Both are gitignored — never `git add` a PNG.
...
- When the ticket adds/changes copy, capture the changed surface in BOTH locales
...
copy — in the state the ticket is about. Seed enough state to exercise the
change (e.g. MORE than one page when the ticket is pagination; a mix of statuses
when the ticket is a filter) and point the browser at that org (set
`localStorage['janusly:activeOrg']` to the seeded org, or seed the dev org
`default`). An empty surface, or a surface showing one row when the feature is
about many, proves nothing.

## Report the evidence honestly

List each PNG path AND the state it shows on the report's Gates / Verification
line, with the DOM-probe result that backs it. If a state was genuinely
unreachable (gated by role / flag / seed you can't satisfy with the dev seed),
say so explicitly and lean on the component/unit test for that state — never
pass off a generic or wrong-state frame as the missing evidence.

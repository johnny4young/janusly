...
- **Verification artifacts stay on disk.** When PHASE 2 verification produces screenshots — Playwright e2e dumps under `test-results/` / `playwright-report/`, browser screenshots taken via the Playwright MCP tools under `.playwright-mcp/`, or any image saved during a manual smoke — DO NOT delete them at the end of the turn. The human reviewer reads them to confirm the change rendered as claimed. Do not clean up `.playwright-mcp/`, do not `rm` test-results directories, do not run any "tidy up" helper that would prune those paths. They stay until the human deletes them. The same applies to any verification log file (curl response captures, `tail -n` outputs piped to a file) the agent created during verification — leave them in place. If a screenshot truly needs to be removed (e.g. it contains a real secret that leaked from the dev env), STOP and flag it to the user instead of deleting silently.

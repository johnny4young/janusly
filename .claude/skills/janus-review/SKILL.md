...
6. **Smoke** when the diff is user-facing. Bring up Compose + api + worker + web with dev headers `x-org-id: default` / `x-user-id: dev-user`, exercise happy path + one error path, verify `mode: "ai"` / `mode: "fallback"` for AI surfaces, tear Compose down at the end. **When the reviewer's OWN inline fixes changed a UI/UX surface** (`apps/web/**` component, panel, toast, overlay, visual affordance, or user-facing copy) the smoke is mandatory AND you MUST leave evidence: drive the changed surface through the real UI (both locales when copy changed), keep console errors at 0, and persist screenshots under `output/review/<ticketname>/` with descriptive names (`web-en-<surface>.png`, `web-es-<surface>.png`, `web-en-<surface>-<state>.png`; `<ticketname>` = the active ENG-NNN slug, e.g. `eng-042`). `output/` is gitignored — never `git add` the PNGs, never delete them. List the PNG paths on the report's **Gates** line. If a gated surface (role / org / flag) can't be reached with the dev seed, say so and lean on the unit/component test — never silently skip. A reviewer UI/UX fix with no smoke + no evidence is unverified.

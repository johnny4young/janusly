---
name: janus-review
description: This skill should be used in the Janusly repository when the user wants to audit staged changes before committing. Specific triggers include "review the staged diff", "review what's staged", "audit before commit", "check the index", "review the implementer's work", "sanity-check changes pre-merge", "review this branch before I commit", "second pair of eyes on this", "look over the staging", "double-check before I push", or "run a pre-commit review". The skill audits `git diff --cached` against AGENTS.md invariants and the approved plan, runs `pnpm build` + `pnpm test` and the typescript-react-reviewer + node review skills, fixes real bugs inline (LEFT UNSTAGED so they show up alongside the implementer's staging in `git diff`), reports design / scope / security findings as discussion blockers, and emits a unified Conventional Commits suggestion. Never runs mutating git — staging stays intact for the human to compare buckets in the IDE. Trigger this whenever the user mentions reviewing, auditing, sanity-checking, or double-checking staged work in Janusly, even if they don't explicitly say "janus-review".
---
...
6. **Smoke** when the diff is user-facing. Bring up Compose + api + worker + web with dev headers `x-org-id: default` / `x-user-id: dev-user`, exercise happy path + one error path, verify `mode: "ai"` / `mode: "fallback"` for AI surfaces, tear Compose down at the end. **When the reviewer's OWN inline fixes changed a UI/UX surface** (`apps/web/**` component, panel, toast, overlay, visual affordance, or user-facing copy) — OR the staged diff is UI-heavy and the implementer left no evidence or only a generic full-page frame — the smoke is mandatory AND you MUST leave FOCUSED evidence following [`references/ui-smoke.md`](references/ui-smoke.md). The five rules are non-negotiable: (1) **focus the ticket's surface** — element screenshot by ref/selector, NEVER a `fullPage` frame of the app shell; (2) **hide unrelated overlays** (onboarding banner, toasts, budget banner, command palette) BEFORE capturing so the change is never obstructed; (3) **capture each state the change introduces** (default / interacted / loading / result / empty / error) as its own PNG — not one screen; (4) **prove every frame shows the change** via a `browser_snapshot` / DOM probe before saving (the probe doubles as the functional assertion); (5) **name `web-<locale>-<surface>-<state>.png`** under `output/review/<ticketname>/` (`<ticketname>` = the active ENG-NNN slug, e.g. `eng-042`), seed real ticket data (not an empty org), capture both locales when copy changed, console 0 errors. `output/` is gitignored — never `git add` the PNGs, never delete them. List each PNG path + the state it shows + its DOM-probe result on the report's **Gates** line. A generic or overlay-covered screenshot is a verification failure — retake it. If a state is genuinely unreachable (role / org / flag the dev seed can't satisfy), say so and lean on the unit/component test — never silently skip, and never pass off a wrong-state frame. A reviewer UI/UX fix with no smoke + no focused evidence is unverified.

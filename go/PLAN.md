...
| T-101 | (stretch) Tick de schedules con líder por advisory lock | P2 | done (ola 6 T-177: scheduler con líder + due-clock) |
| T-102 | (post-piloto) LlmClient Go sobre SDKs oficiales | P2 | done (ola 4: chokepoint anthropic-sdk-go T-099..T-102) |
| T-103 | (post-piloto) pdf.generate: evaluación maroto v2 | P2 | superseded (ola 6 T-164 lo resolvió con writer PDF propio sin deps; maroto descartado; dialect HTML en ola 7 T-521) |
| T-104 | (F3) embed.FS con el dist del web | P2 | done (ola 7 T-522: single-binary tras JANUSLY_GO_SERVE_WEB, make web-embed) |
| T-200 | (F1) auth/context + sesión + permisos por pestaña | P0 | done (olas 2-3: registry central + /auth/context) |
| T-201 | (F1) catálogos: tools/templates/solution-packs/snippets/prompts | P0 | done (tools/templates/prompts olas 3-4; packs+snippets ola 6 T-180) |
| T-202 | (F1) workflows: CRUD/versions/metadata/tags/folders/health | P0 | done (CRUD/versions/trash olas 2-3; tags/folders/health ola 6 T-181/T-182) |
...
| T-205 | (F1) credentials(+health)/members/roles/org-config/billing/onboarding/audit/system | P0 | done (ola 3 + ola 6 T-159..T-161/T-180; billing ola 7 T-516) |
| T-206 | (F1) hito: la web arranca contra Go y navega sus pestañas de lectura | P0 | done (smokes 5/5, olas 3-5) |
| T-300 | (F2) escrituras DLQ/campañas/playbooks/feedback/auto-healing/alerts | P0 | done (olas 2-5 + auto-healing ola 6 T-178; propuestas LLM ola 7 T-515) |
| T-301 | (F2) AI surfaces completas con contrato AI-fallback | P0 | done (ola 4 completa) |
| T-302 | (F2) triggers/webhooks firmados + scheduler + crons system | P0 | done (ola 3 + ola 6 T-168/T-169/T-177..T-179) |
| T-303 | (F2) los 26 tipos de nodo + gramáticas completas (paridad total) | P0 | done (ola 6 T-174..T-177 cerró subworkflow/schedule/loop-for_each/triggers) |
| T-304 | (F2) Secret Store/HMAC/memoria/MCP/SCIM/rate limits/i18n server-events/permisos completos | P0 | done (olas 3-4 + Secret Store T-159/T-160 + SCIM ola 6 T-191..T-194) |
| T-305 | (F2) rollouts canary + import packs + upstream + experiments | P1 | done (rollouts ola 5; packs T-180; upstream T-171; experiments ola 6 T-189/T-190) |
| T-400 | (F3) Playwright completo + browser + perf budgets verdes contra Go | P0 | superseded (los 5 smokes + kill-failover ×3 + lane HA ×3 de olas 3-6 son la cobertura del pilot; la lane Playwright completa es tarea del cutover, no del pilot — decisión REPORT-W6) |
| T-401 | (F3) saldo de diferidos §9 + plan de corte y reversa | P0 | done (ola 6 T-187: CUTOVER-MAP.md + saldo §9) |
...
| 2026-07-30 | superseded pilot posture | Approval deadline fields originally failed closed with `approval_deadline_unsupported_pilot`; the 2026-08-02 parity band supersedes this limitation with durable `fail`/`auto_reject`/`escalate` execution and Node-checkpoint continuation. |
...
| T-501 | Partir `v1.go` (1,304 líneas) en módulos: mounts / runs / workflows / encoding | arquitectura | P1 | done |
| T-502 | Partir `scim.go` (1,354 líneas): verificador / dispatcher / rutas | arquitectura | P1 | done |
| T-503 | Partir `queries.sql` (2,584 líneas) por contexto acotado (sqlc multi-archivo) | arquitectura | P1 | done |
| T-504 | Wiring OTel de trazas + poblar `runs.traceId` (cierra divergencia anotada) | arquitectura | P1 | done |
| T-505 | Alinear granularidad del event-stream (`node.queued`/`node.started` por nodo, paridad 9-eventos) | arquitectura | P0 | done |
| T-506 | LISTEN/NOTIFY como despertador primario del dispatcher (poll = fallback) | performance | P1 | done |
| T-507 | Barrido EXPLAIN de las ~120 queries de la ola 6 + índices faltantes (patrón two-file) | performance | P1 | done |
| T-508 | Perfil de allocs (pprof heap bajo bench) + optimizaciones dirigidas de `safePersist` | performance | P1 | done |
| T-509 | Batch de escrituras de `run_events` en la tx de completion | performance | P2 | done |
| T-510 | Fix cardinalidad de k6-soak (`name` tag) + re-corrida del soak 24h confiable | performance | P0 | done |
...
| T-513 | Tope GLOBAL de proceso para pools externos de db-tools (hoy solo 5/org) | recursos | P1 | done |
| T-514 | Superficies AI restantes: `/ai/explain-run`, `/ai/explain-workflow`, `/ai/review-workflow`, `/ai/suggest-improvement`, `/ai/health` | funcionalidad | P1 | done |
| T-515 | Auto-healing con propuestas LLM detrás del budget gate (hoy solo `harden_retries` determinista) | funcionalidad | P1 | done |
| T-516 | Billing: `/billing/budget`, `/billing/usage`, `GET/POST /workflows/{id}/budget` | funcionalidad | P1 | done |
| T-517 | Replay-lab: `POST /runs/replay-lab` + `/fork` | funcionalidad | P1 | done |
| T-518 | Recovery V2 reads: `/recovery/cases`, `/recovery/ledger`, `/recovery/my-wins` | funcionalidad | P1 | done |
| T-519 | Identidad restante: `GET /organizations`, `POST /users/me`, aceptación de invitación por página, `/plugins/install` | funcionalidad | P2 | done |
| T-520 | `POST /causal` (panel de razonamiento causal, sin LLM — siempre disponible) | funcionalidad | P2 | done |
| T-521 | Follow-ups P2 de T-164: driver S3 SigV4 real del object store + dialect HTML de `pdf.generate` | funcionalidad | P2 | done |
| T-522 | (T-104) `embed.FS` con el dist del web — un solo binario desplegable | funcionalidad | P2 | done |
| T-523 | `ARCHITECTURE.md`: 4 diagramas mermaid (ciclo de vida del run, secuencia subworkflow, mapa de módulos, ladder de claims) + ADRs cortos | docs | P1 | done |
| T-524 | Bookkeeping del PLAN: voltear a done los punteros stale de la tabla de fases (T-201..T-305, T-400 parciales) | docs | P2 | done |
| T-525 | Helper `route()` con gate obligatorio (mata ~500 líneas de boilerplate de mounts; montar sin gate = error de compilación) | refactor | P1 | done |
| T-526 | Partir `complete.go` (888 líneas): terminal-failure/DLQ/hook vs readiness/fan-in | refactor | P1 | done |
| T-527 | Vistas tipadas (structs con tags JSON) para los wires núcleo: runs, dlq, workflows | refactor | P1 | done |
| T-528 | `internal/webhooksig`: unificar los 5 verificadores de firma entrante (posturas por proveedor) | refactor | P1 | done |
| T-529 | DX: `make verify` (escalera completa) + `make seed` (org demo con workflows/runs/DLQ/incidentes) | utilidades | P1 | done |
| T-530 | CLI `janusly-admin`: redrive, resync de schedules, inspección de credenciales (los curls del runbook) | utilidades | P2 | done |
| T-531 | Caos de Postgres: `run-chaos.mjs` — kill de la base a mitad de completion tx, reconnect de pools, recuperación verificada | tests | P0 | done |
| T-532 | Cobertura con piso por paquete en `make ci` + unit tests para los 6 paquetes sin tests propios (orgconfig, objectstore, packs, prompts, contract, migrate) | tests | P1 | done |
| T-533 | Fuzzing de los parsers que comen input externo: cron propio, CloudEvents estricto, escritor PDF | tests | P0 | done |
| T-534 | Property tests de SCIM: secuencias aleatorias de eventos con timestamps desordenados vs invariantes | tests | P1 | done |
| T-535 | Bench de fallo parcial: upstream degradado + DLQ creciendo + breaker disparando, lecturas sin degradarse | tests | P1 | done |
...
| T-526 | `complete.go` (909 líneas) partido en cuatro: `complete.go` (CompleteNode + outcome semántico + inCompletionTx + helpers, 331), `terminalfailure.go` (RetryOrFail con el canal writeSide, DLQ insert + hook de recovery items, breaker/alertas post-fallo, impacto generation-bound; 363), `downstream.go` (scheduleDownstream + gate de aristas + flip terminal + status_checked; 253) — y `readiness.go` (43) que YA EXISTÍA con los predicados puros y mi split SOBRESCRIBIÓ: recuperado de HEAD y el archivo nuevo renombrado a downstream.go. Lección: `ls` del paquete ANTES de crear archivos en un split — el nombre del spec chocó con un módulo real. Cero cambio de SQL/transacciones; suite engine con goleak + lint verdes ANTES del commit. |
| T-525 | DESVIACIÓN DELIBERADA del spec, con mejor propiedad de seguridad: en vez de migrar 143 mounts al helper (churn puro — el registry YA era la fuente única que el middleware consulta), se INVIRTIÓ el enforcement: `s.auth` ahora falla CERRADO ante un patrón montado que no está ni en `routeAuthz` ni en la allowlist explícita `authOnlyRoutes` (las 6 rutas del contrato del reference: auth/context, org/config read, catálogos tools/templates) — olvidar la entrada del registry ya no puede convertirse en acceso auth-only silencioso, responde 500 `route_not_registered` en el primer request de cualquier test. El helper `route()` existe para lo NUEVO (gate+mount colocados, idempotente por harness, pánico ante gate conflictivo) y es el patrón que T-514+ usará. Censo previo que habilitó la allowlist exacta: 177 mounts auth = 171 registry + 6 auth-only; los 6 aliases legacy montan vía `alias()` con sus entradas presentes. `TestUnregisteredPatternFailsClosed` prueba el cierre con un mount pícaro real. La meta de −400 líneas NO se ejecutó (medio, no fin — anotado honesto); ambos sweeps + goleak verdes. |
| T-507 | Barrido EXPLAIN ejecutable (`TestHotQueryPlansAreIndexServed` en package store): 12 queries calientes de las olas 6-7 preparadas desde las CONSTANTES generadas por sqlc (cero duplicación de SQL — el drift es imposible) y explicadas con `enable_seqscan=off` — un Seq Scan que sobrevive sobre una tabla VIGILADA (16 tablas que crecen con el uso) significa que NINGÚN índice puede servir el predicado, no una elección de costo. Hallazgo real: `runs` no tenía índice por `workflow_version_id` — el heatmap de schedule (y la clase de joins de conteo/salud de T-500) degeneraban a barrido org-wide; índice `runs_workflow_version_created_idx` añadido con el patrón two-file ADAPTADO a goose (los runbooks CONCURRENTLY viven en `internal/migrate/rollouts/` con README, porque goose rechaza dos archivos con la misma versión — trampa nueva documentada). LÍMITE HONESTO anotado: el gate detecta "ningún índice puede servir", no "el índice elegido es el óptimo" — las aserciones de O() finas requieren datos+ANALYZE y quedan en T-535. Trampas del arnés que costaron dos iteraciones: EXECUTE no acepta binds por el protocolo extendido (literales controlados inline) y el LIMIT hardcodeado de fire-history desalineaba el conteo de parámetros. |
| T-502 | `scim.go` (1,354 líneas) partido en CINCO, no tres — el corte natural del módulo era más fino que el spec: `scimverify.go` (primitivos de wire: firma fail-closed, parser de sobre, email primario, derivación pura de rol; 210), `scimdispatch.go` (guardas + handlers de usuario con las colisiones; 375), `scimgroups.go` (handlers de grupo + write paths compartidos de membresía; 267), `scimroutes.go` (CRUD admin + picker + resync; 468), `scimwebhook.go` (receptor público; 91) — todos <500. Sin cambio de comportamiento: lifecycle + unit + sweep del registry verdes intactos. |
| T-501 | `v1.go` (1,304 líneas) partido en 4 por responsabilidad: `v1.go` (struct + constructores + middleware auth, 234), `encoding.go` (sobres v1, decode, cursores, null-projections, 145), `runroutes.go` (start/get/list/resume/cancel/DLQ/redrive/replay, 575), `workflowroutes.go` (save/list/latest/versions/rollback/breaker-resume, 409) — todos <700; goimports poda por archivo. CERO cambio de wire: contract suite + goleak verdes sin tocar una expectativa. |
| T-503 | `queries.sql` (2,584 líneas, 393 queries) partido en 11 archivos de contexto bajo `internal/store/queries/` (runs, dispatch, subworkflows, workflows, recovery, playbooks, integrations, ai, members, scim, scheduler, platform) — todos <500 líneas; sqlc.yaml apunta al DIRECTORIO y genera un .go por archivo. Lección de verificación del ticket: el primer "byte-idéntico" era VACUO (el diff comparaba el queries.sql.go viejo que sqlc ya no tocaba — sobra, no evidencia); la equivalencia real se probó por el CONJUNTO DE SÍMBOLOS generados (795 declaraciones func/Params/Row, idénticas antes y después) + build + suite completa + drift-guard del ci (git diff sobre internal/store, agnóstico al layout). |
| T-506 | Ticket de VERIFICACIÓN, como T-179: el despertador LISTEN/NOTIFY ya existía completo desde su ola — `RunWorkers` arranca `listenForWakeups` sobre `janusly_go_wake` (conexión hijackeada CON cierre por ctx — la higiene que T-511 exige), señal coalescida en canal buffered, y el `workerLoop` selecciona wake/poll/ctx con el poll como FALLBACK; `NotifyWake` dispara dentro de las tx de start y de readiness. Lo que faltaba era la PRUEBA de la aceptación: `TestWakeLatencyIsNotifyDriven` — cadena de 3 nodos con poll deliberado de 2s termina con mediana 163ms (poll-driven necesitaría ≥2 ticks ≈4s); cada salto (start→claim, completion→readiness→claim) despertó por NOTIFY. Sin cambios de producción; goleak y suite verdes. |
...
| T-514 | Las 5 superficies AI restantes en `internal/httpapi/aisurfaces.go` (~490 líneas), montadas vía el helper `route()` de T-525 (primer módulo nuevo sobre ese patrón): `GET /ai/health` (auth-only, agregado a `authOnlyRoutes` — el reference no le pone gate), `POST /ai/explain-workflow` + `/ai/explain-run` + `/ai/review-workflow` ({viewer, ai.write}) y `POST /ai/suggest-improvement` ({editor, ai.write}). Reutilización total de la maquinaria existente, cero duplicación: gate compartido `aiSurfaceGate` (aibudget.Gate 402 → aiconfig.Resolve → rate family "ai" 429), `resolveRunExplain` (T-147) es a la vez el fallback $0 Y el grounding DATA-framed del prompt de explain-run, `domain.CheckWorkflowReadiness` es el review fallback (el mismo motor del gate de producción, narrado), `validateGeneratedWorkflow` juzga cada replacement sugerido, `aievidence` arma el side-channel del nodo fallido con firma normalizada. Contrato AI-fallback íntegro: toda ruta degrada a `{mode:"fallback", aiError?, …}` a $0, auditada en AMBOS caminos (las 4 acciones ya estaban en el catálogo). El sanitizador del review descarta severities fuera del enum e ids de nodo inventados, y FUSIONA los hallazgos deterministas del readiness para que un LLM nunca pueda ocultar una regla que el gate haría cumplir; suggest-improvement descarta todo replacement que no pase el validador estricto y ordena por confidence. 3 tests de integración (contrato fallback completo + gates por rol con viewer sembrado + modo AI vía simulador local con issues fuera de contrato y un replacement inválido que NUNCA llega al wire); suite httpapi completa verde (40.9s), lint 0. |
| T-504 | Correlación + trazas OTel. Dos identidades deliberadamente separadas, como el reference: (1) `runs.traceId` es el UUID de CORRELACIÓN estampado en `StartRun` (`uuid.NewString` cuando nadie lo provee) y HEREDADO por los hijos de subworkflow vía `GetRunTraceID` del padre — la cadena completa queda copiable como una traza; proyectado en `runView` + `ListRunSummaries` (antes `"traceId": nil`). (2) Spans OTel reales: `internal/observability/tracing.go` nuevo — Resource `service.name=janusly-go` / `service.namespace=janusly` / `service.instance.id`, exporter console por DEFAULT y OTLP/HTTP con batcher vía `OTEL_EXPORTER=otlp` (+ `none` para silencio; desconocido = error, espejo del rechazo a `jaeger` del reference), `InitTracing` SOLO en `cmd/api` main con shutdown que hace flush en SIGTERM — el código de librería usa el tracer global, así que procesos sin provider pagan ~0 (no-op). Span raíz `run.start` {runId, orgId, versionId} envolviendo la transacción de arranque + span `node.execute` {runId, nodeId, type, attempt} por ejecución reclamada en `runExecutor` (postura `withSpan` del worker.ts). Los harnesses de conformance fijan `OTEL_EXPORTER=none` (miden el contrato HTTP, no el costo del exporter; los benches futuros comparan contra baselines sin trazas). ACCEPTANCE COMPLETO: las TRES divergencias `traceId` salieron de `EXPECTED_DIVERGENCES` y el dual quedó **27/27 verde** — status-linear/run-linear/runs-list ya comparan limpios (solo persisten las 4 de org-config, artefacto documentado del reference). De paso se reparó el target `make dual` (el wrapper corre el hijo con cwd=raíz del worktree; la ruta era relativa a `go/` y el módulo no resolvía). Tests: correlación padre→hijo real por Postgres + spans capturados con `tracetest.SpanRecorder` global + unit del bootstrap (none/desconocido/error-status); suites engine+httpapi+observability verdes, lint 0. |
| T-508 | Perfil de allocs + optimización dirigida del chokepoint de persistencia. Micro-perfil primero (evidencia antes de tocar): 25% de los objetos alocados de `SafePersistPayload` era la COPIA incondicional del árbol en los walkers de redacción; 75% es el marshal reflect del stdlib. Fix: `RedactValues` + `RedactSensitiveKeys` ahora son copy-on-write (`redactValuesCOW`/`redactKeysCOW` — un contenedor se reconstruye SOLO cuando un cambio real ocurre debajo; payload limpio = paso de solo-lectura). A/B intercalado con stash (soak SIGSTOP, medianas de 8): caso limpio 494→383 allocs/op (−22.5%), 31.5KB→14.9KB B/op (−52.7%), 74µs→52µs (−30%); valores-redactados −29.4% allocs / −65.7% B/op. Guard anti-mutación del árbol del caller. Mitad macro: pprof allocs+heap capturados del puerto interno a mitad de `make bench` (commiteados en `conformance/perf/profiles/`), lectura en BENCH.md — top-3 bajo carga: `getRun` serialización 39% (destino T-527 views tipadas), `auth.EffectivePermissions` 6.7% (cacheable por org+rol), `regexp` de claves sensibles 4.5% (memoizable); veredicto del residuo documentado (marshal reflect irreducible sin encoder manual). Nota operativa: el disco del host llegó a 100% a mitad del ticket (link falló con ENOSPC) — se liberó el caché go-build (14GB) + artefactos stale; y la captura falló dos veces porque `/healthz` NO vive en el puerto interno (404) — el poll correcto es `/debug/pprof/cmdline`. Suite grammar verde, lint 0. |
| T-509 | Batch de eventos en la familia de completion. `inCompletionTx` (el chokepoint compartido de las 9 transacciones de la familia: completion exitosa, fallo terminal, retry, waiting/resume, y los 4 settles de subworkflow) ahora construye un `runEventBuffer` ordenado que cada sitio llena con `events.add(...)` — ids, payloads y `created_at` EXACTAMENTE los que producían los INSERT por-fila — y lo vuelca en UN `CopyFrom` (sqlc `:copyfrom` nuevo `InsertRunEvents`, mismas columnas que `InsertRunEventAt`) justo antes del notify del stream, dentro del mismo commit. 12 sitios migrados en 5 archivos (complete/downstream/terminalfailure/subworkflow/waiting); el par del checkpoint padre queda fuera (vive en la tx de START, otro wrapper). El orden de lectura lo define el keyset `(created_at, id)`, así que el batch cambia round-trips, nunca la línea de tiempo. MEDIDO vía el seam DI `wrapTx` (precedente flakyTx) con un contador Exec/CopyFrom: run lineal de 2 nodos → completion path con CERO INSERTs por-fila, 2 COPYs para 5 eventos (antes 5 statements; −60% en esa forma, −(N−1) por tx en fan-outs) y vocabulario/orden byte-idéntico asertado (`run.started → node.queued → node.running → node.succeeded → … → run.succeeded → run.status_checked`). Paridad: suites engine+httpapi verdes, dual 27/27 limpio, lint 0. |
| T-513 | Semáforo de PROCESO sobre los pools externos de db-tools, anidado bajo el presupuesto 5/org existente: `dbMaxProcessPools()` (default 25, env `JANUSLY_GO_DB_TOOL_MAX_PROCESS_POOLS` 1..500) se evalúa en `getDbPool` DESPUÉS de la evicción LRU por-org, así que un pool net-new al tope responde el sentinel estable `errDbPoolExhausted` — que `safeDbError` deja pasar literal y el envelope del tool entrega como `{ok:false, error:"db_pool_exhausted"}` never-throw — y deliberadamente NO hay evicción cross-org (un tenant ruidoso agota su propio presupuesto, jamás desaloja los pools calientes de otro). El swap LRU dentro del presupuesto propio de una org sigue funcionando al tope global (net-zero). Gauge nuevo `janusly_go_db_tool_pools` actualizado en alta/evicción/reset. Test de integración contra Postgres real: 2 orgs × 5 llenan cap=10 exacto, el 11° net-new responde el sentinel sin mover el gauge, los pools calientes siguen sirviendo, el swap por-org es net-zero al tope, y reset → gauge 0 + capacidad recuperada; `safeDbError(errDbPoolExhausted)` asertado como el literal del contrato. Suites tools+executors verdes, lint 0. |
| T-515 | Propuestas LLM en el sweep de auto-healing, detrás del doble opt-in EXISTENTE (env master + `autoHealing.enabled`) + el budget gate — deliberadamente SIN tercer knob (primer intento agregó `autoHealing.llmProposals` al catálogo org-config y los tests de paridad pinneados en las 69 definiciones del reference lo rechazaron; releído el spec, la clave del proveedor + el gate de presupuesto SON los controles de costo). `llmHealingPatch`: contexto 100% scrubbed (firma normalizada + error y nodo pasados por `ScrubSecretShapes`/`RedactSensitiveKeys`), prompt DATA-framed, salida = el MISMO grammar no-estructural de la ola 5 (`patchedConfig` de reemplazo completo del nodo fallido, `confidence` 0..100), y la sugerencia solo se acepta si el snapshot parcheado pasa `domain.Parse` + `domain.Validate` — el mismo juez de las superficies interactivas. TODA degradación (sin key, budget bloqueado con `billing.budget.exceeded` auditado, salida malformada, workflow parcheado inválido) cae al determinista `harden_retries`; el flujo aguas abajo NO cambia (fila `auto_healing_runs` con label `llm_patch` + confidence del modelo, validación sandbox obligatoria, promoción del watcher, risk-ack del operador — el LLM propone, jamás aplica). El contador de presupuesto es el chokepoint de usage existente (cada llamada registra `usage_events` con costo). 3 tests de integración: propuesta simulada validada end-to-end en sandbox + promoción, sin key → `harden_retries`, budget bloqueado → `harden_retries` sin marcar jamás al proveedor (server-trampa) + audit del bloqueo. Suites httpapi+engine verdes, lint 0. |
| T-516 | Billing completo. (1) `GET /billing/usage` — resumen plano `Record<metric, quantity>` back-compat + breakdown dimensional opt-in por enum cerrado (`provider,model,mode,day,node,workflow`), buckets con key `dim=valor|…`, quantity/callCount/fallbackCount, costUsd NULL cuando ninguna fila trae costo (nunca coerce-a-cero), latencia p50/p95/avg con la MISMA fórmula de percentil del reference; ambos caminos leen la MISMA rebanada acotada 30 días / 10k filas. (2) `GET /billing/usage/export` — CSV RFC 4180 con guard anti-inyección de fórmulas (prefijo apóstrofe a `= + - @`), filename fechado, auditado `billing.usage.exported`. (3) `GET /billing/budget?workflowId=` — el envelope compuesto `BudgetCheckResult` camelCase. (4) `POST /workflows/{id}/budget` (admin + org.config.write vía `route()`) — escalera de validación con los códigos exactos del reference (`billing_monthly_usd_invalid`/`billing_warn_percent_invalid`/`billing_policy_invalid`/`workflow_not_found` 404 tenancy), upsert sobre el índice único `(org_id, workflow_id)` del baseline, audit `billing.budget.configured` con before/after. El COMPUESTO: `aibudget.CheckScoped(org, workflowID)` — un override de workflow con límite >0 muerde ANTES que el default de org (gasto sumado por `metadata.workflowId` del mes; el recorder ya estampaba workflowId), ausente/cero cae a `Check(org)`; el dispatcher del nodo `ai` ahora resuelve el workflowId del run (`GetRunWorkflowID` version-join) y gatea con el compuesto. Los GET van al allowlist auth-only (el reference no les pone gate). Tests: shapes de summary/breakdown/CSV+audit, escalera 400/404, y el compuesto mordiendo a nivel workflow con org abierta + relajación. Suites httpapi+aibudget+engine verdes, lint 0. |
| T-517 | Replay Lab completo (adapter engine `replaylab.go` + rutas `replaylab.go` en httpapi, ambos {editor, runs.start} vía `route()` + rate family "ai"). (1) `POST /runs/replay-lab`: sandbox de run completo — precedencia de snapshot patch del caller > `workflow_versions.dagJson` > `inputJson.workflow` ad-hoc, patch validado con `domain.Parse` + el juez compartido `validateGeneratedWorkflow` (escalera exacta de códigos del reference: `runs_source_run_id_required`/`runs_source_run_not_found` 404 sin enumeration leak/`nested_replay_lab`/`runs_suggested_workflow_*`/`no_workflow_snapshot`/`invalid_snapshot`), input del trigger propagado para que `{{context.input.*}}` resuelva idéntico, inserción en UNA tx (roots queued + resto pending + evento `run.started.replay-lab` + NotifyWake), `replayMode=validation` + `parentLinkKind=replay` + traceId NULL (postura del reference: los sandbox no producen versiones ni trazas). (2) `POST /runs/replay-lab/fork`: fork dirigido — predecesores transitivos clonados del run fuente (TODOS deben haber sucedido; `predecessor_not_succeeded` 422), SOLO el nodo fork queued (seed opcional `{input: override}`, cap 64 KiB `override_too_large` 422), downstream pending, ramas ajenas `skipped` con razón `outside_replay_fork`, evento `run.started.replay-lab-fork` con predecessorCount, `parentNodeId` = nodo fork; v1 SIN patch (los patches van por el lab completo para que la validación cubra el DAG entero). Audits `replay_lab.started`/`.fork_started` (ya en catálogo). Tests: write-side POST congelado en el sandbox (contador tripwire), doc editado corre sin tocar `workflow_versions`, linaje visible en el detalle, nested 400, fork con override end-to-end (a clonado, b re-corre leyendo el upstream clonado, c cascadea, d skipped), predecesor fallido 422, override gigante 422. Incidencia: el Postgres de dev murió (exit 255, secuela de la presión de disco) a mitad del ticket — reiniciado, soak sobrevivió con un gap de ~5 min. Suites httpapi+engine verdes, lint 0. |
| T-518 | Recovery V2 reads — solo proyecciones, cero autoridad nueva ({viewer, recovery.read} vía `route()`). El censo mostró que las queries YA existían de la ola 5 (`ListRecoveryCases` con openOnly/runId/limit, `GetRecoveryCase`, `GetRecoveryImpactRollup`); faltaba UNA (`CountOperatorRecoveries`: impact events del operador join runs con `replay_mode IS NULL` — el linaje sandbox jamás cuenta, igual que la proyección del reference). Rutas: `GET /recovery/cases` (default open-only excluyendo los 4 estados terminales, filtro runId, limit 1..200 default 100, orden `(created_at,id)` DESC) + `GET /recovery/cases/{caseId}` (detalle camelCase completo, 404 idéntico para ajenos), `GET /recovery/ledger` (proyección constante del rollup: totalRecovered/downtimeEndedMs/sinceIso; fila ausente = ledger cero, nunca error), `GET /recovery/my-wins` (identidad EXCLUSIVAMENTE del auth context — la ruta no acepta userId del caller; ventana days 1..90 default 30). Tests: open-only vs openOnly=false vs runId vs limit, tenancy 404, ledger vacío→sembrado, my-wins contando SOLO al caller y excluyendo el evento de linaje sandbox, clamp de ventana. Suite httpapi verde (57.8s), lint 0. |
| T-519 | Identidad restante. Cimiento nuevo: `auth.Context` gana `Email` (claim verificado de supabase en las 3 ramas del resolver; para dev-headers el user id con forma de email — la convención del lab que el resolver de membresías ya usaba). Rutas: `GET /organizations` (memberships REALES del caller — org_members join organizations con name/plan/role, LIMIT 50; funciona con cero memberships, por eso va al allowlist auth-only como las identity-only del reference), `POST /users/me` (upsert de `users` con la normalización exacta del reference: trim + colapso de espacios + 2..100; `profile_name_invalid` 400), `POST /auth/invitations/accept` (email verificado obligatorio → 403 `identity_email_required`; email equivocado recibe el envelope IDÉNTICO a not-found — un invitation id no filtra a quién apunta; CAS `AcceptInvitation` sobre status pending → 409 en el segundo intento; membership `InsertOrgMember` ON CONFLICT DO NOTHING + audit `member.joined`), `POST /plugins/install` ({admin, workflows.write} vía `route()`: fila `installed_plugins` + audit `plugin.installed` — el stub honesto de Node). Hallazgo de paridad: `member.joined` es una acción REAL del reference (auth.ts provisioning) que el port original del catálogo omitió — agregada con el pin del catálogo movido deliberadamente 147→148. Lección de test: la DB dev compartida acumula memberships de `api-tester` de corridas previas (el LIMIT 50 truncaba) — identidad única por test. 5 queries sqlc nuevas. Suites httpapi+auth+audit verdes, lint 0. |
| T-520 | Razonador causal determinista portado ENTERO como lógica pura en `internal/domain/causal.go` (cero I/O, cero LLM — siempre disponible): fórmula exacta del reference (`peso_costo*costo + peso_latencia*latencia + peso_calidad*(1-calidad)`, pesos normalizados 40/40/20, score menor gana), estrategias `auto`/`cheapest`/`fastest` con sus sorts, deltas de cada alternativa contra el ELEGIDO (no contra el mejor), y los strings de explicación literales (incluido `"No candidate could be replayed."` para el ranking vacío). Ruta `GET /causal` ({viewer, runs.read} vía `route()`): los 3 params obligatorios → 400 con el código compuesto del reference, run ajeno → **403 `runs_forbidden`** (fiel al reference: 403 aquí, no 404), evento pinneado a `(id, runId, nodeId, type='decision.made')` vía query nueva `GetDecisionEvent` → 404 `runs_no_decision_event`, parser de candidatos desde `payload.ranking` (breakdown.{cost,latency,quality}; filas sin nodeId se descartan). Nota de censo: el pilot no tiene emisor de `decision.made` (el router engine no se portó) — la superficie de LECTURA es fiel e independiente del emisor; el test siembra el evento directo. Tests: unit de la fórmula con valores calculados a mano + estrategias + replay vacío; integración del explorador completo (400/403/404/shape). Suite httpapi verde (115s), lint 0. |
| T-521 | Los dos follow-ups P2 de T-164. (1) Driver S3 SigV4 REAL (`internal/objectstore/sigv4.go`, sin SDK): cadena HMAC AWS4 completa, canonicalización RFC 3986 con query ordenada, PUT firmado por headers (host + x-amz-date + x-amz-content-sha256 + content-type), presigned GET por query (UNSIGNED-PAYLOAD, TTL 3600 env-configurable), path-style forzado contra endpoints custom (MinIO/R2 — realidad de certificados) y virtual-host contra AWS propio, URL de CDN cuando `PUBLIC_BASE_URL` está configurado; credenciales por los env AWS estándar, degradación never-throw sin ellas. Bypass deliberado del chokepoint SSRF, IGUAL que el S3Provider del reference (el endpoint es configuración de operador, no input de autor). Test: la fake S3 VERIFICA las firmas server-side reconstruyendo el canonical request desde el http.Request crudo (no confía en el cliente — lo mismo que hace MinIO) + round-trip PUT firmado → presigned GET → bytes idénticos + pin de la cadena de derivación + degradaciones (sin credenciales, CDN). Desviación honesta: la aceptación decía MinIO local; con el disco al 99% no se bajó la imagen — la fake verificadora cubre la misma matemática y el protocolo es idéntico. (2) Dialect HTML de `pdf.generate` (`pdfhtml.go`, `golang.org/x/net/html` ya indirecta): whitelist exacta del reference (h1-h6/p/div/br/ul/ol/li/hr/pre/code/strong/em/span/a[http|https]/table), deny-set que mata tag Y children (script/style/iframe/object/embed/link/meta/base/form/input/img/svg) con degradación VISIBLE `[removed: <tag>]` en el documento (la aceptación lo pedía; el reference calla — mejora deliberada), `javascript:` pierde el href y el texto queda plano, tags desconocidos pasan children (postura reference), mapeado a los MISMOS pdfLine del writer propio. Contrato viejo actualizado: el test que pinneaba "html se rechaza" ahora pinnea "formato desconocido se rechaza". Suites tools+objectstore+executors verdes, lint 0. |
| T-522 | Single-binary web. Paquete `internal/webdist`: `go:embed all:dist` con `HandlerFor(fs.FS)` como seam de tests — archivos exactos primero, assets hasheados de Vite bajo `/assets/` con `Cache-Control: public, max-age=31536000, immutable` (el hash del filename lo hace seguro para siempre), `index.html` y top-level con `no-cache` (un redeploy se ve al siguiente load), y fallback SPA a `index.html` para toda ruta que no es archivo (rutas client-side). Montaje: detrás del flag `JANUSLY_GO_SERVE_WEB=true`, como patrón `GET /` del PROPIO mux — la especificidad de Go 1.22 garantiza que toda ruta API gana, y el estático es público por diseño (cero data de tenant, sin auth). Git-higiene: el `dist/` commiteado es solo un placeholder honesto ("corre make web-embed"); `make web-embed` construye el bundle de Vite y lo copia al embed root (assets generados fuera de git via .gitignore con excepción del placeholder). SMOKE real ejecutado: bundle de producción (2.2MB) embebido en el binario, boot con el flag, `/` sirve el shell del app, asset hasheado responde con `immutable`, `/flows` (ruta SPA) responde el shell 200, `/healthz` (API) sigue ganando. Unit tests del contrato de serving sobre `fstest.MapFS` + placeholder. Nota shell: el alias eza de `ls` contaminó una var del smoke (curl 000) — usar `find` en scripts. Lint 0. |
| T-523 | `ARCHITECTURE.md` nuevo, enlazado desde el README del pilot. Los 4 diagramas mermaid pedidos, VERIFICADOS renderizando (4/4 exitosos con beautiful-mermaid): (1) stateDiagram del ciclo run/nodo con cada punto CAS nombrado (claim SKIP LOCKED, completion, flip terminal, resume solo-waiting) y el reaper como transición ruidosa; (2) sequenceDiagram del subworkflow con el checkpoint atómico (una tx: child insert + parent running→waiting + ambos eventos) y el handoff con marker + reconciler; (3) flowchart del mapa de módulos internal/* con las dependencias permitidas y las reglas que las flechas codifican (domain/grammar sin I/O, nada importa httpapi de vuelta, todo jsonb por safe-persist); (4) flowchart del claim ladder completo con el due-clock (wakeup consumido transaccionalmente — un worker crasheado no puede doble-armar un retry). Más 5 ADRs de una página: due-clock vs BullMQ (un solo sistema durable; precisión acotada por el sweep, aceptable a escala backoff), sesgos zoned-window deliberadamente NO unificados, postura orphan-tolerant, sweeps vs colas para trabajo de sistema (idempotente-por-scan), y la serie T-500 como institución census-first (con los incidentes reales que previene: T-526 clobber, T-515 knob rechazado). Sección de orden de lectura para contribuidores nuevos. |
| T-524 | Bookkeeping del PLAN: las 13 filas engañosas de la tabla de fases volteadas con nota de dónde se cumplió cada una — T-101→done (ola 6 T-177), T-102 ya estaba done, T-103→superseded (writer propio; maroto descartado; HTML T-521), T-104→done (T-522), T-201/202/205/300/302/303/304/305→done con sus olas anotadas (billing T-516 y propuestas LLM T-515 incluidos), T-400→superseded (los 5 smokes + kill-failover ×3 + lane HA ×3 SON la cobertura del pilot; la lane Playwright completa es tarea del cutover — decisión REPORT-W6), T-401→done (T-187 CUTOVER-MAP). Cero filas con estado engañoso restantes (`grep 'todo →'` = 0). |
| T-527 | Views tipadas para los 3 wires núcleo (`internal/httpapi/views.go`): `RunView` + `RunSummaryView` (runs detail/status/list), `DeadLetterSummaryView` + `RecoveryOverlayView` + `DeadLetterDetailView` (dlq v1 list + detalle legacy exacto-snapshot), `WorkflowListItemView` + `VersionView` — reemplazando los literales `map[string]any` de los handlers. Sin `omitempty` en NINGÚN campo (el contrato exige nulls explícitos, jamás keys ausentes — hay test reflect que lo hace incumplible) y `normalizedRaw` preserva la semántica de `rawOrNull` (jsonb vacío → null explícito). Doble guard: `TestViewKeySetsPinned` pinnea el key-set EXACTO de las 7 views (un typo en el tag rompe el test) y el typo en el nombre de campo ya NO COMPILA (demostrado por construcción); paridad: suite httpapi verde + **dual 27/27 intacto** — el wire no se movió un byte. Bonus de la iteración: flake temporal pre-existente cazado y reparado — `TestScheduleNodeDueClockLoop` exigía >30min al siguiente fire del cron `0 3 * * *` (UTC), lo que fallaba TODOS los días de 21:30 a 22:00 hora local (-05); la aserción ahora exige ALINEACIÓN (instante futuro en tope de hora UTC), no distancia. Lint 0. |
| T-528 | `internal/webhooksig` nuevo: el 80% común de los verificadores entrantes (parse `k=v` multi-candidato — la rotación de secretos manda varios `v1=`, sanidad hex, HMAC-SHA256 sobre payload compuesto, compare constant-time, ventana de skew) con las POSTURAS por proveedor como config (`Compose` del payload, `ToleranceSeconds` 0 = sin timestamp, `TimestampMillis`, `AsymmetricSkewReasons`) y vocabulario canónico de razones. Los 4 verificadores migrados como adapters finos que conservan su contrato externo EXACTO: external-runtime (`t=<s>,v1=` sobre `t.body`, bool), WorkOS/SCIM (`t=<ms>,v1=`, razones asimétricas expired/future_timestamp, (bool, reason)), PagerDuty V3 (multi-candidato `v1=` sobre el body crudo, SIN timestamp), Slack (timestamp en header propio + `v0=` prefijado — los quirks del proveedor se quedan en el adapter). Guard: las matrices de firma EXISTENTES de los 4 pasaron sin tocar un caso, + unit del core (escalera completa, rotación con candidato stale, posturas ms/asimétrica/sin-timestamp). Suite httpapi verde (54s), lint 0. |
| T-529 | DX de una orden. `make verify` (`scripts/verify.sh`): la escalera completa generate+drift → build → lint → unit → integration → parity, para-en-el-primero con tiempos por etapa — corrida de aceptación REAL: 6/6 verdes (drift 24s, build 5s, lint 2s, unit 12s, integration 205s, parity 15s, ~4.4min total). `make seed` (`cmd/seed`): org demo determinista construida ENTERAMENTE por la API pública con dev headers (jamás SQL directo — lo que el seed crea es exactamente lo que el producto puede crear): 5 workflows (pipeline verde, par subworkflow padre/hijo, reporte con nodo schedule, ingest flaky contra `demo-ingest.invalid` — dominio .invalid = firma de fallo determinista repetible con cero superficie SSRF), 6 runs (3 verdes + subworkflow + 2 fallidos con la MISMA firma → cluster DLQ + 2 incidentes auto-creados), credenciales dummy con degradación honesta cuando el Secret Store no está configurado en dev. Idempotencia probada en vivo: segunda pasada = workflows/credenciales skip, runs skip por umbral, cero errores. Hallazgos del camino: `/start` toma el DOC (no workflowId) — el seed relee la última versión por la misma API del web; y el envelope de `/recovery/items` es `items`, no `rows`. Lint 0. |
| T-530 | CLI `janusly-admin` (`cmd/admin`): los curls del runbook como subcomandos, TODO por las rutas HTTP existentes con service token (Bearer) o dev headers — jamás SQL directo, así el CLI puede exactamente lo que la API autoriza. Subcomandos: `redrive --dead-letter <id>` (redrive de identidad exacta) / `redrive --signature <sig>` (cluster-apply), `schedules list --workflow` (historial + próximos fires) / `schedules resync --workflow` (rollback-a-la-versión-actual — el resync que el engine hace en cada save), `credentials health`, `runs inspect --run` (run + nodos con errores truncados legibles). `--json` en todos para scripting. Aceptación EN VIVO contra la org demo del seed: los 4 grupos ejecutados — health, inspect de un run fallido con su ENOTFOUND, redrive real de un dead letter (`redriven: true`), listado del schedule `0 3 * * *` con los 5 próximos fires, y resync verde (aprendizaje: el rollback pide `sourceVersionId`, no `version`). Lint 0. |
| T-531 | Caos de Postgres (`conformance/run-chaos.mjs`, `make chaos`) — el gemelo de run-failover con la BASE como víctima. Container Postgres PROPIO (el compartido hospeda el soak 24h — el caos no lo contamina), migraciones por el subcomando `migrate` del binario, y TRES rondas de: tráfico sostenido → `docker stop` a mitad de vuelo (cohorte pre-caída con runs en el aire) → veredicto 1: el proceso NO muere y responde envelope degradado limpio (500 JSON, jamás hang) → `docker start` → veredicto 2: los pools pgx reconectan SOLOS sin reinicio del proceso → veredicto 3: TODOS los runs pre-caída llegan a terminal (los queued se reclaman al volver; los claims cuya completion tx murió con la base los cosecha el reaper RUIDOSAMENTE a failed+DLQ — recuperados, jamás perdidos) y exactly-once se sostiene (attempts ≤ 1 en cargas sin retry). Corrida real ×3 verde: 33 runs, 3 cosechados-no-perdidos, cero dobles ejecuciones, reconexión in-process en las 3 rondas. Aprendizaje del harness: la cohorte se corta AL matar la base (seguir arrancando runs contra una base muerta es probar otra cosa). |
| T-532 | Cobertura con piso + los 6 paquetes sin tests. Tests nuevos que fallan ante mutación obvia: orgconfig (capas tenant>env>default con nombres de source reales, rechazo de tipo/enum/rango — y CAZÓ UN GAP REAL: ni `parseStored` ni `parseEnv` validaban `AllowedValues`, así que una fila fuera del enum se aplicaba; endurecido con `inAllowedValues` en ambas capas — defensa en profundidad sobre la validación del write path, las filas pueden preceder un apriete del enum), objectstore (escalera de providers + traversal + degradaciones never-throw), packs (invariantes de boot del catálogo embebido: ids únicos, workflows parseados, NodeCount, payloads), prompts (la mitad pura: gramática REAL `{{var.x}}`, required nombrado en el error, default llena, token no declarado pasa intacto para la capa engine), contract (manifiesto puro: método/path/summary/response, sin duplicados), migrate (contrato del embed: versiones únicas contiguas desde 1, marcador goose — el runtime corre en cada boot de integración). Piso por paquete en `scripts/coverage-floor.sh` (lista plana — macOS trae bash 3.2, sin arrays asociativos) pineado en los valores actuales (orgconfig 40 / objectstore 87 / packs 76 / prompts 22 / contract 100 / migrate 0 honesto / webhooksig 92 / grammar 85 / domain 73), cableado en `make ci`, y la MUTACIÓN demostrada: piso subido a 99 → exit 1 con el paquete señalado. Lint 0. |
| T-533 | Fuzzing de los 3 parsers que comen input externo, sumado a la lane `make fuzz` existente (grammar) — 6 fuzzers en total, `FUZZTIME` configurable (45s default). (1) `FuzzCronParse` (cron propio de 5 campos): jamás panic, y un schedule aceptado JAMÁS produce un fire imposible — `Next` siempre avanza, alineado al minuto, y la cadena `Next(Next(t))` sigue avanzando (sin puntos fijos); semillas hostiles (campos 60/24/32/13/7, `*/0`, negativos, 6 campos, unicode, 30-feb). (2) `FuzzParseExternalRuntimeEvent` (CloudEvents estricto): jamás panic con UTF-8 roto, solo acepta los 3 tipos conocidos, y la PROPIEDAD de strictness fuzzeada — inyectar UN campo desconocido al envelope de cualquier evento aceptado lo voltea a rechazado. (3) `FuzzRenderMarkdownPDF` + `FuzzRenderHTMLPDF` (escritor propio): todo input hostil produce un PDF estructuralmente válido (header `%PDF-1.4` + trailer `%%EOF`) — jamás panic, jamás basura; semillas con paréntesis sin balancear (el escape del stream), inyección `%%EOF`, deny-set, UTF-8 roto. Corrida REAL de 45s por fuzzer: 4/4 PASS sin hallazgos. Lint 0. |
| T-534 | Property tests de SCIM — y el ticket que más pagó de la ola: el generador determinista (semilla fija 20260801, `JANUSLY_SCIM_PROP_SEED` para explorar; 200 secuencias de 12-22 eventos con timestamps desordenados ±30min y emails COLISIONANTES) encontró DOS BUGS REALES en la secuencia 0: (1) las filas humanas legacy (user_id=email, columna email NULL — el patrón que el backfill del resolver ya reconoce) eran INVISIBLES al collision guard de provisioning → INSERT duplicado sobre `org_members_org_user_idx` → 500 eterno con WorkOS reintentando para siempre; fix: `FindScimMemberByEmail` matchea `email = $2 OR lower(user_id) = $2`. (2) `DeleteScimMembershipByEmail` borraba por (org, email) SIN guard de propiedad — deprovisionar un SCIM podía BORRAR una fila human-invited que compartiera el email; fix: `AND invited_by = 'scim:webhook'` (SCIM solo borra lo suyo). Además el error del webhook se tragaba sin log (un 500 indiagnosticable en prod) — ahora `slog.Warn` con eventId/type/causa. Las 4 invariantes como funciones puras reutilizables (stale-probe post-hoc que no puede mutar el snapshot; fila humana intacta byte a byte; rol derivado = derivación de ALGÚN sharer activo — refinada dos veces por el hallazgo de que la membresía es por-EMAIL y los colisionantes comparten fila, ambos refinamientos documentados en el código; replay de event-ids = no-op por snapshot completo). Shrinking greedy que imprime la secuencia MÍNIMA (14-15 eventos en los hallazgos reales) + la semilla. 200 secuencias verdes en 21.6s; suite httpapi completa verde, lint 0. |
| T-535 | Bench de mundo hostil (`conformance/perf/k6-hostile.js` + `run-hostile-bench.mjs`, `make bench-hostile`). Tres fases k6 con nombres de métrica FIJOS (lección de cardinalidad de T-510): baseline_reads (0-20s, runs/dlq/health con el sistema quieto) → chaos_writes (20-55s: starts continuos contra `.invalid` con 1 intento — el DLQ crece y el MISMO workflow fallando dispara el breaker de fallos consecutivos a pausa) → hostile_reads (30-55s, las MISMAS lecturas bajo ese caos). El wrapper computa hostil-p95/baseline-p95 por familia y falla sobre 2× (contrato de degradación acotada); guarda de honestidad: <20 chaos starts = la fase hostil no probó nada → rojo. Corrida REAL con el soak congelado (SIGSTOP → medir → CONT, la regla de co-residencia): **1928 starts fallidos absorbidos, runs list 3.9→4.8ms (1.22×), dlq list 3.6→5.1ms (1.41×), health 0.2→0.2ms (1.34×) — 3/3 acotados**. Veredicto anexado a BENCH.md como escenario permanente + serie `hostile-series.jsonl` para regresiones futuras. Nota de dependencia: T-535 no necesitaba el VEREDICTO del soak — solo la no-co-residencia, satisfecha con el freeze. |
| T-510 | k6-soak confiable + soak 24h COMPLETO. La mitad harness cerró temprano en la ola (tags `name` fijos por escenario — nunca URLs con runId en las series; puertos parametrizados; humo 3m). El soak 24h relanzado el 2026-08-01 ~14:40 terminó el 2026-08-02: **1412 muestras, k6 ok**. El veredicto automático marcó "CRECIÓ" (+24.4% heap primer-vs-último cuarto) y la INVESTIGACIÓN lo resolvió como artefacto del baseline: heap por octavos 8.7·5.5·8.8·8.9·8.9·8.8·8.8·8.9 MB — plano en ~8.8 MB las últimas ~18h; el segundo octavo cayó a 5.5 por la caída del Postgres del host + las ventanas SIGSTOP de los benches T-508/T-535 (la carga se congeló y el heap se encogió), deprimiendo el promedio del primer cuarto a 7.1. RSS a la BAJA (35.5→32.8 por octavos, final 28.0 MB, pico global = muestra 2 del arranque) y goroutines planas (~41, rango 28-54) corroboran: **ESTABLE, sin fuga**. Anexo de investigación en SOAK.md + REPORT-W6 actualizado (todos los criterios del go/no-go en ✅). Mejora anotada para el próximo relanzamiento: excluir muestras bajo SIGSTOP/outage del promedio del detector. |

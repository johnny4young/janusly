...
import (
	...
	"github.com/johnny4young/janusly/go/internal/packs"
...
func (s *V1Server) mountProductSurfaceRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /snippets", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"snippets": views}))
	}))

	mux.HandleFunc("POST /snippets", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		var body snippetBody
		if err := decodeBody(r, &body); err != nil {
			writeLegacy(w, opError(http.StatusUnprocessableEntity, "snippet_invalid", "invalid snippet body", nil))
			return
		}
		if message := validateSnippetBody(&body); message != "" {
			writeLegacy(w, opError(http.StatusUnprocessableEntity, "snippet_invalid", message, nil))
			return
		}
		q := store.New(s.pool)
		if _, err := q.FindSnippetByName(r.Context(), store.FindSnippetByNameParams{
			...
			writeLegacy(w, opError(http.StatusConflict, "snippet_name_exists", "A snippet with that name already exists", nil))
			return
		}
		...
		writeLegacy(w, opResult{status: http.StatusCreated, data: map[string]any{"snippet": customSnippetView(row)}})
	}))
	...
	mux.HandleFunc("POST /snippets/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if strings.HasPrefix(id, builtinSnippetPrefix) {
			writeLegacy(w, opError(http.StatusConflict, "snippet_builtin_immutable", "Built-in snippets are read-only", nil))
			return
		}
		var body snippetBody
		if err := decodeBody(r, &body); err != nil {
		...
		if err != nil {
			writeLegacy(w, opError(http.StatusNotFound, "snippet_not_found", "Snippet not found", nil))
			return
		}
		...
		writeLegacy(w, opOK(map[string]any{"snippet": customSnippetView(row)}))
	}))
	...
	mux.HandleFunc("GET /solution-packs", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"packs": views}))
	}))
	...
	mux.HandleFunc("GET /solution-packs/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"pack": view}))
	}))
	...
	mux.HandleFunc("POST /workflows/import-pack", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil || body.PackID == "" {
			writeLegacy(w, opError(http.StatusBadRequest, "solution_pack_invalid_request", "packId is required", nil))
			return
		}
		...
		writeLegacy(w, opResult{status: http.StatusCreated, data: map[string]any{
	...
	mux.HandleFunc("GET /onboarding", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeLegacy(w, s.loadOnboardingState(r, rc))
	}))
	mux.HandleFunc("POST /onboarding", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil ||
			...
			writeLegacy(w, opError(http.StatusBadRequest, "onboarding_invalid_action", "Unknown onboarding action", nil))
			return
		}
		q := store.New(s.pool)

...
import (
	...
	"github.com/johnny4young/janusly/go/internal/experiment"
...
func (s *V1Server) mountEvalRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /eval/datasets", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"datasets": views}))
	}))
	...
	mux.HandleFunc("POST /eval/datasets", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.Name) == "" ||
			...
			writeLegacy(w, opError(http.StatusBadRequest, "eval_dataset_name_required", "Invalid eval dataset body", nil))
			return
		}
		...
		if _, err := q.FindEvalDatasetByName(r.Context(), store.FindEvalDatasetByNameParams{
			...
			writeLegacy(w, opError(http.StatusConflict, "eval_dataset_name_exists",
		...
		writeLegacy(w, opResult{status: http.StatusCreated, data: map[string]any{"dataset": evalDatasetView(dataset)}})
	}))
	...
	mux.HandleFunc("GET /eval/datasets/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			writeLegacy(w, opError(http.StatusNotFound, "eval_dataset_not_found", "Eval dataset not found", nil))
			return
		}
		...
		writeLegacy(w, opOK(map[string]any{"dataset": evalDatasetView(dataset), "examples": examples}))
	}))
	...
	mux.HandleFunc("GET /eval/datasets/{id}/export", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if format != "jsonl" && format != "json" {
			writeLegacy(w, opError(http.StatusBadRequest, "eval_dataset_unknown_format",
	...
	mux.HandleFunc("GET /experiments", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"experiments": views}))
	}))
	...
	mux.HandleFunc("GET /experiments/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			writeLegacy(w, opError(http.StatusNotFound, "experiment_not_found", "Experiment not found", nil))
			return
		}
		writeLegacy(w, opOK(map[string]any{"experiment": experimentView(row)}))
	}))
	...
	mux.HandleFunc("POST /experiments/run", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.Name) == "" ||
			...
			writeLegacy(w, opError(http.StatusBadRequest, "experiment_invalid_body",
		...
		if !experiment.ScorerKinds[body.ScorerKind] {
			writeLegacy(w, opError(http.StatusBadRequest, "experiment_invalid_body", "unknown scorerKind", nil))
			return
		}
		ctx := r.Context()
		q := store.New(s.pool)
		...
		writeLegacy(w, opOK(map[string]any{"experimentId": experimentID, "summary": summary}))
	}))
}

...
func (s *V1Server) mountUpstreamHealthRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /upstream/sources", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeLegacy(w, opOK(map[string]any{"sources": views}))
	}))
	...
	mux.HandleFunc("POST /upstream/sources", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			if isUniqueViolation(err) {
				writeLegacy(w, opError(http.StatusConflict, "upstream_source_conflict", "source name already exists", nil))
				return
			}
			writeLegacy(w, opError(http.StatusInternalServerError, "internal_error", "Internal error", nil))
			return
		...
		writeLegacy(w, opResult{status: http.StatusCreated, data: map[string]any{"source": upstreamSourceView(row)}})
	}))
	...
	mux.HandleFunc("POST /upstream/sources/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			...
			writeLegacy(w, opError(http.StatusNotFound, "upstream_source_not_found", "source not found", nil))
			return
		}
		...
		writeLegacy(w, opOK(map[string]any{"source": upstreamSourceView(row)}))
	}))

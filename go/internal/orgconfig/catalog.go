...
var Definitions = []Definition{
	...
	{Key: "ai.timeoutMs", Category: "ai", Description: "LLM request timeout in milliseconds.", ValueType: "number", Default: float64(30000), EnvKeys: []string{"OPENAI_TIMEOUT_MS"}, Min: floatPtr(1)},
	{Key: "ai.maxRetries", Category: "ai", Description: "AI SDK retry count for LLM calls.", ValueType: "number", Default: float64(2), EnvKeys: []string{"OPENAI_MAX_RETRIES"}, Min: floatPtr(0)},
	{Key: "ai.maxOutputUnits", Category: "ai", Description: "Maximum output tokens allowed for one LLM provider call. Bounds spend and oversized replies without changing the provider-specific prompt contract.", ValueType: "number", Default: float64(4096), EnvKeys: []string{"JANUSLY_LLM_MAX_OUTPUT_UNITS"}, Min: floatPtr(256), Max: floatPtr(16384)},
	{Key: "ai.promptMaxChars", Category: "ai", Description: "Maximum prompt length accepted by AI endpoints.", ValueType: "number", Default: float64(4000), EnvKeys: []string{"AI_PROMPT_MAX_CHARS"}, Min: floatPtr(1)},
	{Key: "ai.rateLimitPerMin", Category: "ai", Description: "Per-org AI request limit per minute.", ValueType: "number", Default: float64(30), EnvKeys: []string{"AI_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	{Key: "ai.budgetMonthlyUsd", Category: "ai", Description: "Monthly USD ceiling for org-wide AI spend. 0 disables the gate (no budget).", ValueType: "number", Default: float64(0), EnvKeys: []string{"JANUSLY_AI_BUDGET_MONTHLY_USD"}, Min: floatPtr(0), Fractional: true},
	{Key: "ai.budgetWarnPercent", Category: "ai", Description: "Percent of the monthly budget at which the operator gets a billing.budget.warned audit row + Recovery Center banner.", ValueType: "number", Default: float64(80), EnvKeys: []string{"JANUSLY_AI_BUDGET_WARN_PERCENT"}, Min: floatPtr(0), Max: floatPtr(100)},
	...
	{Key: "http.timeoutMs", Category: "http", Description: "Default outbound HTTP timeout budget in milliseconds.", ValueType: "number", Default: float64(30000), EnvKeys: []string{"JANUSLY_HTTP_TIMEOUT_MS"}, Min: floatPtr(1)},
	{Key: "http.maxResponseBytes", Category: "http", Description: "Default maximum decoded body size for HTTP nodes and http.request.", ValueType: "number", Default: float64(1000000), EnvKeys: []string{"JANUSLY_HTTP_MAX_RESPONSE_BYTES"}, Min: floatPtr(1)},
	{Key: "http.maxRedirects", Category: "http", Description: "Default maximum redirect hops for outbound HTTP.", ValueType: "number", Default: float64(5), EnvKeys: []string{"JANUSLY_HTTP_MAX_REDIRECTS"}, Min: floatPtr(0)},
	{Key: "http.streamPreviewBytes", Category: "http", Description: "How many bytes of a streamed HTTP response body get captured into the persisted node output for audit when `bodyMode: \"stream\"` is set on an `http` node or `http.request` tool. The full response still flows through the byte cap (`http.maxResponseBytes`); only the preview is what survives into `run_nodes.state_json`. Range 1024..1048576 (1 KB..1 MB), default 65536 (64 KB).", ValueType: "number", Default: float64(65536), EnvKeys: []string{"JANUSLY_HTTP_STREAM_PREVIEW_BYTES"}, Min: floatPtr(1024), Max: floatPtr(1048576)},
	...
	{Key: "email.rateLimitPerMin", Category: "email", Description: "Per-org email.send limit per minute.", ValueType: "number", Default: float64(100), EnvKeys: []string{"JANUSLY_EMAIL_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	...
	{Key: "subworkflow.maxDepth", Category: "runs", Description: "Maximum nested subworkflow depth.", ValueType: "number", Default: float64(5), EnvKeys: []string{"JANUSLY_MAX_SUBWORKFLOW_DEPTH"}, Min: floatPtr(1)},
	{Key: "runs.streamMaxSubscriptions", Category: "runs", Description: "Maximum concurrent live-run SSE subscriptions per organization (per API replica).", ValueType: "number", Default: float64(50), EnvKeys: []string{"JANUSLY_STREAM_MAX_SUBSCRIPTIONS"}, Min: floatPtr(1), Max: floatPtr(1000)},
	{Key: "runs.circuitBreakerThreshold", Category: "runs", Description: "Consecutive failed ordinary runs that pause a workflow via its circuit breaker. Range 2..100, default 5 (a threshold of 1 would be a hair trigger). A per-workflow `config.recovery.circuitBreaker.consecutiveFailures` overrides this; setting that to `false` opts a workflow out entirely.", ValueType: "number", Default: float64(5), Min: floatPtr(2), Max: floatPtr(100)},
	{Key: "runs.humanFormResumeTtlSeconds", Category: "runs", Description: "Lifetime of newly-issued human-form resume tokens, in seconds. Range 300..604800 (5 minutes..7 days), default 604800. Each token carries its signed expiry, so changing this affects only tokens issued afterwards.", ValueType: "number", Default: float64(604800), Min: floatPtr(300), Max: floatPtr(604800)},
	...
	{Key: "mcp.clientRateLimitPerMin", Category: "mcp", Description: "Per-org rate limit for external `mcp_tool` invocations (per minute, applied as `mcp_client.<alias>.<toolName>` bucket).", ValueType: "number", Default: float64(60), EnvKeys: []string{"JANUSLY_MCP_CLIENT_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	...
	{Key: "mcp.stdioMaxLifetimeMs", Category: "mcp", Description: "Max wall-clock lifetime in milliseconds for a stdio MCP child process. The sandbox layer kills the child (SIGTERM then SIGKILL after a short grace) past this. Range 60000..3600000 (1 min..1 h).", ValueType: "number", Default: float64(600000), EnvKeys: []string{"JANUSLY_MCP_STDIO_MAX_LIFETIME_MS"}, Min: floatPtr(60000), Max: floatPtr(3600000)},
	...
	{Key: "mcp.stdioMaxVmKb", Category: "mcp", Description: "Max virtual-memory size (KB) enforced via Linux ulimit -v on stdio MCP children. Applied only on Linux when JANUSLY_MCP_SANDBOX_ENFORCE_LINUX is true; macOS / Windows ignore this value. Range 131072..4194304 (128 MiB..4 GiB).", ValueType: "number", Default: float64(524288), EnvKeys: []string{"JANUSLY_MCP_STDIO_MAX_VM_KB"}, Min: floatPtr(131072), Max: floatPtr(4194304)},
	{Key: "slack.rateLimitPerMin", Category: "integrations", Description: "Per-org slack.post tool calls per minute.", ValueType: "number", Default: float64(60), EnvKeys: []string{"JANUSLY_SLACK_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	{Key: "github.rateLimitPerMin", Category: "integrations", Description: "Per-org github.create_issue tool calls per minute.", ValueType: "number", Default: float64(60), EnvKeys: []string{"JANUSLY_GITHUB_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	{Key: "webhook.rateLimitPerMin", Category: "integrations", Description: "Per-org webhook.send tool calls per minute.", ValueType: "number", Default: float64(120), EnvKeys: []string{"JANUSLY_WEBHOOK_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	{Key: "pdf.rateLimitPerMin", Category: "integrations", Description: "Per-org pdf.generate tool calls per minute.", ValueType: "number", Default: float64(30), EnvKeys: []string{"JANUSLY_PDF_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	{Key: "db.rateLimitPerMin", Category: "integrations", Description: "Per-org db.schema.describe / db.query.* tool calls per minute.", ValueType: "number", Default: float64(60), EnvKeys: []string{"JANUSLY_DB_TOOL_RATE_LIMIT_PER_MIN"}, Min: floatPtr(1)},
	...
	{Key: "auth.sessionTtlSeconds", Category: "auth", Description: "Per-org override for the WorkOS browser-session TTL (in seconds). Range 300..86400 (5 minutes..24 hours). Default 28800 (8 hours). The SSO callback reads this when it creates the revocable server-side session, so changing it only affects newly-created sessions.", ValueType: "number", Default: float64(28800), Min: floatPtr(300), Max: floatPtr(86400)},
	...
	{Key: "memory.recallMaxEntries", Category: "memory", Description: "Hard cap on the number of memory entries returned per recall call. Range 1..32, default 8. See docs/memory-policy.md.", ValueType: "number", Default: float64(8), Min: floatPtr(1), Max: floatPtr(32)},
	{Key: "memory.recallMaxBytes", Category: "memory", Description: "Hard cap on the total bytes returned per recall call. Range 1024..65536 (1 KiB..64 KiB), default 8192. See docs/memory-policy.md.", ValueType: "number", Default: float64(8192), Min: floatPtr(1024), Max: floatPtr(65536)},
	...
	{Key: "autoHealing.maxAttemptsPerSignature", Category: "auto-healing", Description: "Loop-breaker ceiling: the same normalized failure signature can be auto-healed at most this many times inside the loop window. The count is over prior auto_healing_runs rows for (orgId, signature) and is captured at diagnose time. Range 1..10, default 3.", ValueType: "number", Default: float64(3), Min: floatPtr(1), Max: floatPtr(10)},
	{Key: "autoHealing.loopWindowDays", Category: "auto-healing", Description: "Loop-breaker window in days: prior auto_healing_runs rows for (orgId, signature) older than this are not counted against the per-signature cap. Range 1..90, default 14.", ValueType: "number", Default: float64(14), Min: floatPtr(1), Max: floatPtr(90)},

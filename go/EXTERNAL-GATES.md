...
5. **`cutover`** — frozen mutation ingress, stopped Node producers, a passing
   exact-tree Node-to-Go queue handoff, active Go header and metric, zero
   ownership overlap, the applied proxy configuration hash, and a smoke run.
6. **`canary`** — Go retains global mutation ownership while read routing
   ...
   all stop thresholds must pass without automatic rollback.

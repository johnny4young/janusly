...
func (s *V1Server) slackCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	if reason != "" {
		writeUnversioned(w, opError(http.StatusUnauthorized, "slack_interaction_invalid_signature", "invalid Slack signature", nil))
		return
	}
	...
	if interaction == nil {
		...
		writeUnversioned(w, opError(http.StatusBadRequest, "slack_interaction_invalid_request", "invalid Slack interaction payload", nil))
		return
	}
	if interaction.TeamID != connection.TeamID {
		rejected("team_mismatch", map[string]any{"signedTeamId": interaction.TeamID})
		writeUnversioned(w, opError(http.StatusForbidden, "slack_interaction_unauthorized", "Slack team is not authorized", nil))
		return
	}
	var mappings []slackMapping
	...
	if mappedUserID == "" {
		...
		writeUnversioned(w, opError(http.StatusForbidden, "slack_interaction_unauthorized", "Slack user is not authorized", nil))
		return
	}
	...
	if !permitted {
		...
		writeUnversioned(w, opError(http.StatusForbidden, "slack_interaction_unauthorized", "mapped user lacks recovery permission", nil))
		return
	}
	...
	if claimed == 0 {
		writeUnversioned(w, opOK(map[string]any{"ok": true, "duplicate": true}))
		return
	}
	...
	if err != nil {
		writeUnversioned(w, opError(http.StatusNotFound, "recovery_item_not_found", "recovery item not found", nil))
		return
	}
	...
	if moved == 0 {
		writeUnversioned(w, opError(http.StatusConflict, "recovery_item_transition_invalid",
	...
	writeUnversioned(w, opOK(map[string]any{"ok": true, "response_type": "ephemeral", "text": message}))
}

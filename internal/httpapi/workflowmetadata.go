...
func (s *V1Server) mountWorkflowMetadataRoutes(mux *http.ServeMux) {
	s.route(mux, "GET /workflows/{workflowId}/metadata", routeGate{auth.RoleViewer, "workflows.read"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.getWorkflowsMetadataCore(r, rc))
	})

	s.route(mux, "POST /workflows/{workflowId}/metadata", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsMetadataCore(r, rc))
	})

	s.route(mux, "POST /workflows/{workflowId}/slo", routeGate{auth.RoleAdmin, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsSloCore(r, rc))
	})

	// The NARROW folder route: only the folder column moves.
	s.route(mux, "POST /workflows/{workflowId}/folder", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsFolderCore(r, rc))
	})

	// The NARROW per-row tag route.
	s.route(mux, "POST /workflows/{workflowId}/tags", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsTagsCore(r, rc))
	})

	// Distinct dropdown feeds — tombstoned workflows excluded.
	s.route(mux, "GET /workflows/tags", routeGate{auth.RoleViewer, "workflows.read"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.getWorkflowsTagsCore(r, rc))
	})
	s.route(mux, "GET /workflows/folders", routeGate{auth.RoleViewer, "workflows.read"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.getWorkflowsFoldersCore(r, rc))
	})

	// Bulk collection ops — org-scoped in the repo, audited with counts.
	s.route(mux, "POST /workflows/folders/rename", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsFoldersRenameCore(r, rc))
	})

	s.route(mux, "POST /workflows/folders/delete", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsFoldersDeleteCore(r, rc))
	})

	s.route(mux, "POST /workflows/folders/assign", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsFoldersAssignCore(r, rc))
	})

	s.route(mux, "POST /workflows/tags/assign", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsTagsAssignCore(r, rc))
	})

	s.route(mux, "POST /workflows/tags/rename", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsTagsRenameCore(r, rc))
	})

	s.route(mux, "POST /workflows/tags/delete", routeGate{auth.RoleEditor, "workflows.write"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		writeUnversioned(w, s.postWorkflowsTagsDeleteCore(r, rc))
	})
}
...
func (s *V1Server) getWorkflowsMetadataCore(r *http.Request, rc v1Request) opResult {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			...
			return opOK(map[string]any{"metadata": nil})
		}
		return opError(http.StatusInternalServerError, "internal_error", "Internal error", nil)
	}
	return opOK(map[string]any{"metadata": workflowMetadataView(row)})

}

func (s *V1Server) postWorkflowsMetadataCore(r *http.Request, rc v1Request) opResult {
	workflowID := r.PathValue("workflowId")
	if !s.ownsActiveWorkflow(r, rc.orgID, workflowID) {
		return opError(http.StatusNotFound, "workflow_not_found", "Workflow not found", nil)
	}
	...
	if err := decodeBody(r, &body); err != nil || body.Metadata == nil {
		return opError(http.StatusUnprocessableEntity, "workflow_metadata_invalid", "invalid workflow metadata body", nil)
	}
	metadata := body.Metadata
	if message := validateMetadataBody(metadata); message != "" {
		return opError(http.StatusUnprocessableEntity, "workflow_metadata_invalid", message, nil)
	}
	...
	return opOK(map[string]any{"metadata": view})

}

func (s *V1Server) postWorkflowsSloCore(r *http.Request, rc v1Request) opResult {
	return s.setWorkflowSloCore(r, rc, r.PathValue("workflowId"))

}

func (s *V1Server) postWorkflowsFolderCore(r *http.Request, rc v1Request) opResult {
	workflowID := r.PathValue("workflowId")
	if !s.ownsActiveWorkflow(r, rc.orgID, workflowID) {
		return opError(http.StatusNotFound, "workflow_not_found", "Workflow not found", nil)
	}
	...
	if err := decodeBody(r, &body); err != nil || len(body.Folder) > 120 {
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "invalid folder body", nil)
	}
...
func (s *V1Server) postWorkflowsTagsCore(r *http.Request, rc v1Request) opResult {
	workflowID := r.PathValue("workflowId")
	if !s.ownsActiveWorkflow(r, rc.orgID, workflowID) {
		return opError(http.StatusNotFound, "workflow_not_found", "Workflow not found", nil)
	}
	...
	if err := decodeBody(r, &body); err != nil || body.Tags == nil || len(*body.Tags) > 30 {
		return opError(http.StatusBadRequest, "workflow_metadata_invalid",
			"body must carry a tags array; this route replaces the whole list", nil)
	}
	for _, tag := range *body.Tags {
		if strings.TrimSpace(tag) == "" || len(tag) > 60 {
			return opError(http.StatusBadRequest, "workflow_metadata_invalid", "tags must be 1..60 characters", nil)
		}
	}
...
func (s *V1Server) getWorkflowsTagsCore(r *http.Request, rc v1Request) opResult {
	tags, err := store.New(s.pool).ListDistinctWorkflowTags(r.Context(), rc.orgID)
	if err != nil {
		return opError(http.StatusInternalServerError, "internal_error", "Internal error", nil)
	}
	return opOK(map[string]any{"tags": orEmptySlice(tags)})

}

func (s *V1Server) getWorkflowsFoldersCore(r *http.Request, rc v1Request) opResult {
	folders, err := store.New(s.pool).ListDistinctWorkflowFolders(r.Context(), rc.orgID)
	if err != nil {
		return opError(http.StatusInternalServerError, "internal_error", "Internal error", nil)
	}
	return opOK(map[string]any{"folders": orEmptySlice(folders)})

}

func (s *V1Server) postWorkflowsFoldersRenameCore(r *http.Request, rc v1Request) opResult {
	...
	if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.From) == "" ||
		...
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "from and to are required", nil)
	}
...
func (s *V1Server) postWorkflowsFoldersDeleteCore(r *http.Request, rc v1Request) opResult {
	var body struct{ Folder string }
	if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.Folder) == "" {
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "folder is required", nil)
	}
...
func (s *V1Server) postWorkflowsFoldersAssignCore(r *http.Request, rc v1Request) opResult {
	var body struct {
	...
	if err := decodeBody(r, &body); err != nil || len(body.WorkflowIDs) == 0 ||
		...
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "workflowIds (1..100) required", nil)
	}
	q := store.New(s.pool)
...
func (s *V1Server) postWorkflowsTagsAssignCore(r *http.Request, rc v1Request) opResult {
	var body struct {
		WorkflowIDs []string `json:"workflowIds"`
		Tag         string   `json:"tag"`
	...
	if err := decodeBody(r, &body); err != nil || len(body.WorkflowIDs) == 0 ||
		...
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "workflowIds and tag required", nil)
	}
	q := store.New(s.pool)
	owned, err := q.ListOwnedActiveWorkflowIDs(r.Context(), store.ListOwnedActiveWorkflowIDsParams{
		OrgID: rc.orgID, Ids: body.WorkflowIDs,
...
func (s *V1Server) postWorkflowsTagsRenameCore(r *http.Request, rc v1Request) opResult {
	var body struct{ From, To string }
	if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.From) == "" ||
...
func (s *V1Server) postWorkflowsTagsDeleteCore(r *http.Request, rc v1Request) opResult {
	var body struct{ Tag string }
	if err := decodeBody(r, &body); err != nil || strings.TrimSpace(body.Tag) == "" {
		return opError(http.StatusBadRequest, "workflow_metadata_invalid", "tag is required", nil)
	}

...
func writeUnversionedValue(w http.ResponseWriter, value any) {
...
func (s *V1Server) startSso(w http.ResponseWriter, r *http.Request) {
	...
	if callbackURL == "" {
		writeUnversioned(w, opError(http.StatusInternalServerError, "sso_callback_not_configured",
	...
	if errors.Is(err, pgx.ErrNoRows) || (err == nil && connection.Status != "active") {
		writeUnversioned(w, opError(http.StatusNotFound, "sso_no_active_connection",
...
func (s *V1Server) callbackSso(w http.ResponseWriter, r *http.Request) {
	...
	if callbackURL == "" || webBaseURL == "" {
		writeUnversioned(w, opError(http.StatusInternalServerError, "sso_not_configured", "SSO is not configured", nil))
		return
	}
	code := r.URL.Query().Get("code")
	stateToken := r.URL.Query().Get("state")
	if code == "" || stateToken == "" {
		writeUnversioned(w, opError(http.StatusBadRequest, "sso_code_and_state_required", "code and state are required", nil))
		return
	}
	envelope, err := ssostate.Verify(stateToken)
	if err != nil {
		s.invalidSsoState(r.Context(), "default", "invalid_signature_or_expiry")
		writeUnversioned(w, opError(http.StatusBadRequest, "sso_invalid_state", "invalid state", nil))
		return
	}
	...
	if s.workos == nil {
		...
		writeUnversioned(w, opError(http.StatusBadRequest, "sso_exchange_failed", "SSO exchange failed", nil))
		return
	}
	...
	if profile.ConnectionID != connection.ProviderConnectionID {
		...
		writeUnversioned(w, opError(http.StatusBadRequest, "sso_connection_mismatch", "SSO connection mismatch", nil))
		return
	}
	...
	if !decision.Allowed {
		...
		writeUnversioned(w, opError(http.StatusForbidden, "sso_policy_violation",
	...
	if err != nil {
		...
		writeUnversioned(w, opError(http.StatusInternalServerError, "sso_membership_persist_failed",

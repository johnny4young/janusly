...
func TestUnversionedRoutesMatchVersionedData(t *testing.T) {
	h := newAPIHarness(t)
	workflowID := "unversioned-read-" + h.org
	workflow := map[string]any{
		"id": workflowID, "name": "Unversioned read contract",
	...
	for _, pair := range pairs {
		unversionedStatus, _, unversioned := callJSON(t, h, pair[0], true)
		versionedStatus, headers, versioned := callJSON(t, h, pair[1], true)
		if unversionedStatus != http.StatusOK || versionedStatus != http.StatusOK {
			t.Fatalf("%s/%s statuses %d/%d", pair[0], pair[1], unversionedStatus, versionedStatus)
		}

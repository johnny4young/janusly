...
func (s *V1Server) mountScimRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /org/scim/directories", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeUnversioned(w, opOK(map[string]any{"directories": views}))
	}))
	...
	mux.HandleFunc("POST /org/scim/directories", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_invalid_body", "invalid body", nil))
			return
		}
		providerDirectoryID := strings.TrimSpace(body.ProviderDirectoryID)
		if providerDirectoryID == "" {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_provider_directory_id_required",
		...
		if body.DefaultRole != "" {
			if !isScimBuiltinRole(body.DefaultRole) {
				writeUnversioned(w, opError(http.StatusBadRequest, "scim_default_role_invalid",
		...
		if _, attached, err := s.getScimDirectoryForOrg(r.Context(), rc.orgID); err != nil {
			...
			writeUnversioned(w, opError(http.StatusConflict, "scim_directory_already_attached",
		...
		writeUnversioned(w, opOK(map[string]any{"directory": scimDirectoryView(row)}))
	}))
	...
	mux.HandleFunc("POST /org/scim/directories/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if body.Status != nil {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_directory_status_immutable",
				"use DELETE /org/scim/directories/{id} to revoke a directory", nil))
			return
		}
		if body.DefaultRole == nil {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_no_updatable_fields",
		...
		if err != nil {
			writeUnversioned(w, opError(http.StatusNotFound, "scim_directory_not_found", "SCIM directory not found", nil))
			return
		}
	...
	mux.HandleFunc("GET /org/scim/groups", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if !attached {
			writeUnversioned(w, opOK(map[string]any{"groups": []map[string]any{}}))
			return
		}
		...
		writeUnversioned(w, opOK(map[string]any{"groups": views}))
	}))
	...
	mux.HandleFunc("GET /org/scim/group-role-mappings", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		writeUnversioned(w, opOK(map[string]any{"mappings": views}))
	}))
	...
	mux.HandleFunc("POST /org/scim/group-role-mappings", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if !attached {
			writeUnversioned(w, opError(http.StatusConflict, "scim_directory_required_for_mappings",
				"attach a SCIM directory before configuring group role mappings", nil))
			return
		}
		providerGroupID := strings.TrimSpace(body.ProviderGroupID)
		if providerGroupID == "" {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_provider_group_id_required",
				"providerGroupId is required (e.g. directory_group_…)", nil))
			return
		}
		if !isScimBuiltinRole(body.Role) {
			writeUnversioned(w, opError(http.StatusBadRequest, "scim_role_invalid",
		...
		if _, err := q.GetScimGroupState(r.Context(), store.GetScimGroupStateParams{
			...
			writeUnversioned(w, opError(http.StatusNotFound, "scim_unknown_provider_group_id",
		...
		if _, err := q.FindScimGroupRoleMappingByGroup(r.Context(), store.FindScimGroupRoleMappingByGroupParams{
			...
			writeUnversioned(w, opError(http.StatusConflict, "scim_group_role_mapping_exists",
		...
		writeUnversioned(w, opOK(map[string]any{"mapping": scimMappingView(row)}))
	}))
	...
	mux.HandleFunc("POST /org/scim/group-role-mappings/{id}", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			writeUnversioned(w, opError(http.StatusNotFound, "scim_group_role_mapping_not_found",
	...
	mux.HandleFunc("POST /org/scim/resync", s.auth(func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if !attached {
			writeUnversioned(w, opError(http.StatusConflict, "scim_directory_required_for_resync",

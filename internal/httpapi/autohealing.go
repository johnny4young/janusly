...
func (s *V1Server) mountAutoHealingRoutes(mux *http.ServeMux) {
	...
	s.route(mux, "GET /auto-healing/{id}", routeGate{auth.RoleViewer, "autohealing.read"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err != nil {
			writeUnversioned(w, opError(http.StatusNotFound, "autoheal_not_found", "Not found", nil))
			return
		}
	...
	s.route(mux, "POST /auto-healing/{id}/decide", routeGate{auth.RoleEditor, "autohealing.decide"}, func(w http.ResponseWriter, r *http.Request, rc v1Request) {
		...
		if err := decodeBody(r, &body); err != nil || body.Accepted == nil || len(body.Comment) > 2000 {
			writeUnversioned(w, opError(http.StatusBadRequest, "autoheal_invalid_body", "invalid body", nil))
			return
		}
		q := store.New(s.pool)
		row, err := q.GetAutoHealingRun(r.Context(), store.GetAutoHealingRunParams{
		...
		if row.Status != "validated" {
			writeUnversioned(w, opError(http.StatusConflict, "autoheal_not_pending",
		...
		if !*body.Accepted {
			...
			if err != nil || declined == 0 {
				writeUnversioned(w, opError(http.StatusConflict, "autoheal_already_resolved",
			...
			writeUnversioned(w, opOK(map[string]any{"ok": true, "accepted": false}))
			return
		}
		...
		if (evidence == nil || evidence == "writes_skipped" || evidence == "static") && !body.AcknowledgeValidationRisk {
			writeUnversioned(w, opError(http.StatusConflict, "autoheal_validation_risk_ack_required",

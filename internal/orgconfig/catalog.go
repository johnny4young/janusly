...
var Definitions = []Definition{
	...
	{Key: "mcp.clientCommandAllowlist", Category: "mcp", Description: "Comma-separated list of executable commands that stdio MCP connections in this org may spawn (e.g. \"node,uvx,npx\"). An explicit empty tenant value denies all stdio commands, even when JANUSLY_MCP_ALLOWED_COMMANDS is set. Only an absent or invalid tenant value falls back to that process environment setting.", ValueType: "string", Default: "", EnvKeys: []string{"JANUSLY_MCP_ALLOWED_COMMANDS"}, AllowEmpty: true},
	...
	{Key: "memory.embeddingModel", Category: "memory", Description: "Ollama model id used to compute embeddings for memory entries. An explicit empty tenant value selects the built-in bge-m3 model; an absent or invalid tenant value falls back to JANUSLY_EMBEDDING_MODEL. Stored on each entry for explicit re-embedding when the model changes. See docs/memory-policy.md.", ValueType: "string", Default: "", EnvKeys: []string{"JANUSLY_EMBEDDING_MODEL"}, AllowEmpty: true, Validate: validateEmbeddingModel},
	{Key: "memory.embeddingBaseUrl", Category: "memory", Description: "Base URL of the embedding service when the operator runs an external instance (e.g. a remote Ollama box). An explicit empty tenant value selects the built-in http://ollama:11434 URL; an absent or invalid tenant value falls back to OLLAMA_BASE_URL. A value set here is org-admin input, so the embedding fetch enforces the outbound SSRF policy (validation + DNS pinning); only the operator env default may point at private infrastructure.", ValueType: "string", Default: "", EnvKeys: []string{"OLLAMA_BASE_URL"}, AllowEmpty: true, Validate: validateEmbeddingBaseURL},
	...
	{Key: "autoHealing.autoApply", Category: "auto-healing", Description: "Reserved setting; currently has no effect. Validated auto-healing proposals require explicit operator application. Enabling this key does not authorize automatic writes.", ValueType: "boolean", Default: false},

...
var incompatibleSnapshots = []struct {
	name         string
	field        []string
	errorExcerpt string
}{
	{name: "unsupported DSL version", field: []string{"workflow", "dslVersion"}, errorExcerpt: "dslVersion"},
	{name: "unknown node type", field: []string{"workflow", "nodes", "0", "type"}, errorExcerpt: "unsupported"},
}

func TestIncompatibleSnapshotFailsBeforeExecution(t *testing.T) {
	for _, test := range incompatibleSnapshots {
		...
		RETURNING id, attempts`, runID).Scan(&claim.RowID, &claim.Attempt); err != nil {
			...
			reloaded.executeClaim(ctx, claim, execute, quietLogger())
			if executed != 0 {
				t.Fatalf("incompatible snapshot invoked the executor %d times", executed)
			}
		...
		WHERE r.id=$1`, runID).Scan(&runStatus, &rootStatus, &afterStatus, &afterAttempts,
			...
			if runStatus != "failed" || rootStatus != "failed" || afterStatus != "pending" || afterAttempts != 0 {
				t.Fatalf("incompatible snapshot advanced: run=%s root=%s successor=%s attempts=%d",
					runStatus, rootStatus, afterStatus, afterAttempts)
			}
			if !strings.Contains(message, "run workflow snapshot invalid") || !strings.Contains(message, test.errorExcerpt) {
				t.Fatalf("failure must explain the incompatible snapshot: %q", message)
			}
...
func TestIncompatibleSnapshotFailsDueTimer(t *testing.T) {
	for _, test := range incompatibleSnapshots {
		t.Run(test.name, func(t *testing.T) {
			ctx, pool, eng, org := newHarness(t)
			doc := `{"nodes":[
		{"id":"pause","type":"wait_until","config":{"duration":"PT1H"}},
		{"id":"after","type":"noop","config":{}}
			...
			claim := ClaimedNode{RunID: runID, NodeID: "pause", OrgID: org}
			if err := pool.QueryRow(ctx, `UPDATE run_nodes SET status='running', started_at=now()
		WHERE run_id=$1 AND node_id='pause' AND status='queued'
		RETURNING id, attempts`, runID).Scan(&claim.RowID, &claim.Attempt); err != nil {
				t.Fatalf("claim timer: %v", err)
			}
		...
		WHERE id=$1`, runID, test.field); err != nil {
			...
			due, err := pool.Exec(ctx, `UPDATE run_wakeups SET wake_at=now()-interval '1 second'
		WHERE run_node_id=$1 AND reason='wait_until'`, claim.RowID)
			if err != nil || due.RowsAffected() != 1 {
				t.Fatalf("fixture must own a persisted wait_until timer: rows=%d err=%v", due.RowsAffected(), err)
			}
			// The second sweep must find nothing left to retry or duplicate.
			eng.processDueWaitingWakeups(ctx, store.New(pool))
			eng.processDueWaitingWakeups(ctx, store.New(pool))
			var runStatus, timerStatus, afterStatus, message string
			var deadLetters, failureEvents, wakeups int
			if err := pool.QueryRow(ctx, `SELECT r.status, timer.status, after_node.status,
		timer.error_json->>'message',
		(SELECT count(*) FROM dead_letters WHERE run_id=r.id),
		(SELECT count(*) FROM run_events WHERE run_id=r.id AND type='node.failed'),
		(SELECT count(*) FROM run_wakeups WHERE run_node_id=timer.id)
		FROM runs r
		JOIN run_nodes timer ON timer.run_id=r.id AND timer.node_id='pause'
		JOIN run_nodes after_node ON after_node.run_id=r.id AND after_node.node_id='after'
		WHERE r.id=$1`, runID).Scan(&runStatus, &timerStatus, &afterStatus, &message,
				&deadLetters, &failureEvents, &wakeups); err != nil {
				t.Fatalf("read failed timer: %v", err)
			}
			if runStatus != "failed" || timerStatus != "failed" || afterStatus != "pending" {
				t.Fatalf("incompatible timer advanced: run=%s timer=%s successor=%s",
					runStatus, timerStatus, afterStatus)
			}
			if !strings.Contains(message, "run workflow snapshot invalid") || !strings.Contains(message, test.errorExcerpt) {
				t.Fatalf("failure must explain the incompatible snapshot: %q", message)
			}
			if deadLetters != 1 || failureEvents != 1 || wakeups != 0 {
				t.Fatalf("timer failure must persist once and clear its clock: dead letters=%d failures=%d wakeups=%d",
					deadLetters, failureEvents, wakeups)
			}
		})
	}
...
func TestIncompatibleSnapshotRejectsApprovalWithoutMutation(t *testing.T) {
	for _, test := range incompatibleSnapshots {
		...
		WHERE id=$1`, runID, test.field); err != nil {
			...
			if !errors.Is(err, ErrRunSnapshotInvalid) || !strings.Contains(err.Error(), test.errorExcerpt) {

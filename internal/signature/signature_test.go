...
func TestNormalizeRules(t *testing.T) {
	cases := []struct {
		...
		{"worker stall ignores per-run timestamp",
			map[string]any{"code": "worker_stalled", "message": "Node was left running since 2026-08-03T10:01:02Z"}, Context{NodeType: "tool"},
			Result{"Worker stalled on tool node", "unknown", "platform"}},

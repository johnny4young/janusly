...
func TestRuntimeDbPoolCap(t *testing.T) {
	...
	for org := range 2 {
		for cred := range 5 {
			if _, err := getIdleDbPool(cache, ctx, fmt.Sprintf("cap-org-%d", org), fmt.Sprintf("cred-%d", cred), dsn); err != nil {
	...
	if _, err := getIdleDbPool(cache, ctx, "cap-org-2", "cred-0", dsn); !errors.Is(err, errDbPoolExhausted) {
	...
	if pool, err := getIdleDbPool(cache, ctx, "cap-org-0", "cred-3", dsn); err != nil || pool == nil {
	...
	if _, err := getIdleDbPool(cache, ctx, "cap-org-0", "cred-5", dsn); err != nil {
	...
	// Closing drains everything and the gauge follows.
	cache.Close()
	if got := testutil.ToFloat64(metricDbToolPools); got != 0 {
		t.Fatalf("close must zero the gauge, got %v", got)
	}
	cache = testDBPools(t, 10)
	if _, err := getIdleDbPool(cache, ctx, "cap-org-2", "cred-0", dsn); err != nil {
		t.Fatalf("a new owner must start with free capacity: %v", err)
	}
}

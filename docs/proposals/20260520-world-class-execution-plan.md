...
| Live SSE (ENG-137) leaks events across orgs through a misrouted Redis channel. | Channel naming is `janusly:run-events:<orgId>:<runId>`; the publisher writes to `<orgId>` and the subscriber verifies `auth.orgId` matches the channel suffix before delivering. The cross-org isolation test is explicit in the AC. |

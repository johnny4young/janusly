...
**Status: Shipped.** Derive-on-read: a single `onboarding_progress` `(orgId,userId)` cache + a self-fetching `OnboardingBanner` overlay; `GET /onboarding` derives the six milestones live from durable org-state (credentials / succeeded run / DLQ / replayed-or-resolved DLQ / `workflow.pack_imported` audit), so ZERO event hooks into ENG-129/credentials/dlq/engine. New `onboarding` permission category, 4 audit actions, 1 error code; AI-unavailable never deadlocks (structural + flag).

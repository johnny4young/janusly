...
This is the **client** path: Janusly consuming external MCP servers as workflow steps via the `mcp_tool` node. The sibling **server** path (Janusly exposing 18 read tools plus 10 consent-gated write tools over stdio MCP) lives in `packages/mcp-server` and is unrelated to anything documented here.

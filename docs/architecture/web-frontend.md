...
solution packs behind one search. Activity contains Runs, Recover, Reasoning,
and Agent activity; Settings contains Workspace, Connections, Team, and Tools.
...
absent. Runs owns execution history, active-run evidence, usage, explanations,
and sandbox forks. Recover owns waiting human actions, failed-node actions,
and the dead-letter/replay queue. Both projections consume the same App-owned
run state; Recover is not a second polling or run-authority path.

...
**Run workspace:** Activity's Runs section mounts `RunWorkspace`, which composes the existing `RunsPanel`, `ReasoningPanel`, and `MultiAgentTimeline` projections behind accessible Overview / Timeline / Agents tabs. It consumes the active run data already owned by `App` and MUST NOT introduce another fetch or run-identity authority. Arrow keys plus Home/End implement roving tab focus; Timeline and Agents remain disabled until a run is active, and an active-run identity change resets the selected inner view to Overview before the new run can render. The Activity contextual rail exposes Runs, Recover, Reasoning, and Agent activity, while the command palette and explicit full-view actions keep direct expert routes stable. Keep those direct routes compatible with persisted `ActiveTab` state.
...
workflows, Build with AI, Configure, Recipes, Packs, and Experiments; Activity
contains Runs, Recover, Reasoning, and Agent activity; Settings contains
Workspace, Connections, Team, and Tools. Permission filtering is delegated to
the existing `canOpenTab` policy and opening a destination selects its first
permitted section. Home has no redundant section rail. The command palette
keeps direct expert routes while putting the four destinations first.
...
authoring-category and collapsed state; obsolete group keys are ignored. The
workflow identity card, AI status strip, and complete node palette render only
for the two authoring tabs (`copilot` and `inspector`); operational
destinations never carry unrelated draft controls. Workflows keeps a visible
New workflow action that routes authorized users into Build with AI and
remains disabled when `workflows.write` is absent. Runs owns execution history,
active-run evidence, usage, explanations, and sandbox forks. Recover owns
waiting human actions, failed-node actions, and the dead-letter/replay queue.
Both projections consume the same App-owned run state; Recover is not a second
polling or run-authority path.

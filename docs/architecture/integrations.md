...
`packages/engine/src/integration-tools.ts` is the stable compatibility barrel
used by the registry and existing consumers. Provider ownership lives under
`packages/engine/src/integration-tooling/`:

- `slack.ts` owns the Incoming Webhook schema and executor;
- `github.ts` owns issue creation and issue-comment schemas and executors;
- `pagerduty.ts` owns incident read, deterministic policy evaluation,
  acknowledge, snooze, and the safe-default working-window helper;
- `webhook.ts` owns the generic signed webhook contract and HMAC formatter;
- `shared.ts` is the only credential-resolution, rate-limit, usage-recorder,
  and bounded parsing/error helper chokepoint.

Provider modules may depend on `shared.ts`; they do not depend on each other or
import the compatibility barrel. The internal directory is deliberately not
named `integration-tools/` so extensionless ESM imports continue to resolve
`integration-tools.ts`. `integration-tools-modules.test.ts` fixes the ten
runtime exports, exact eight registered tools and `writeSide` posture, closed
module inventory, dependency direction, and absence of vendor SDK imports.
...
bit and bounded `inputFields` descriptors derived from the same registered
`ToolDefinition` as execution. `writeSide` tells the browser that some valid
invocations can mutate external state; it does not replace input-sensitive
runtime classification. `inputFields` carries only field name, primitive/JSON
kind, required state, and optional string enum—the planner-only JSON Schema
stays private. The workflow Inspector keeps the current selection visible
while searching, labels read-only versus write-capable tools before execution,
and replaces a previous tool's input with the new tool's bounded example (or
`{}`) when the contract changes. Schema-guided edits preserve unknown keys and
complete runtime expressions; Advanced JSON remains the exact-object escape
hatch. `ToolPicker` plus the `ToolInputEditor` dispatch boundary are shared by

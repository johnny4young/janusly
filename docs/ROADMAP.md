...
| ENG-010 | Vitest browser mode for canvas tests | Partial | P3 | — | Jsdom can't execute React Flow canvas paths reliably (drag, resize, edge layout). Want Vitest browser mode for canvas-level component tests. AC: a few representative React Flow tests run in browser mode in CI. **Remaining:** Local browser-mode pool exists via `vitest.browser.config.ts` + `pnpm test:browser`, with 6 `<WorkflowCanvas>` tests covering render+counts, click→onNodeClick, fitView dimensions+transform, validation overlay, Controls toolbar, and Background pattern. Wire `pnpm test:browser` into CI when ENG-015 creates the workflow so the CI portion of the AC is satisfied. |

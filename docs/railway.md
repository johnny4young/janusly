...
make supply-chain IMAGE=janusly:qualification
```
...
For persisted data, follow the
[binary/snapshot/schema compatibility matrix](local-deployment.md#binary-snapshot-and-schema-compatibility)
before replacing an image or restoring a backup. This local OCI check does not
establish a schema upgrade path or production RPO/RTO.
The supply-chain target adds checksummed BuildKit provenance metadata, the
saved image, OCI/runtime identity, and a standalone SPDX 2.3 SBOM generated by
a digest-pinned Syft image. This bundle is deliberately unsigned; publishing
the image and signing registry attestations require a separate release action.

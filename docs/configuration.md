...
| `memory.allowedKinds` | none | `""` | `packages/data/src/memoryEntriesRepo.ts` | CSV of writable memory kinds: `recovery_rationale`, `run_summary`, `runbook_fragment`, `patch_rationale`, `generated_workflow`, `workflow_vector`. Empty means memory is enabled but no kind is writable yet. |

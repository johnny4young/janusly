...
### §4.0 Status Update (2026-06-10)

The deferred multi-provider verification ran (ENG-194): `scripts/model-eval-compare.ts` is now provider-aware and compared OpenAI against the Haiku free-JSON baseline in one sweep (10 prompts × 3 samples per config, blind Sonnet judge). Results: Haiku free-JSON 87% ai-mode / judge 73.65 / $0.0070 per run; gpt-4o-mini 93% / 63.75 / $0.0006; gpt-4.1-mini 73% / 68.95 / $0.0017. gpt-4o-mini exceeds the baseline on reliability at roughly 1/12 the cost but loses ~13% relative judge quality, concentrated on the complex business prompts — the pre-declared bar (reliability AND quality within ~10%) was not met, so **the Anthropic-only completion posture stands** and no tenant enable happened. The verification also surfaced and fixed a real wire bug: the OpenAI JSON-mode option in `LlmClient` was silently ignored by the AI SDK's Responses-API path; JSON mode is now applied via spec-level `responseFormat` middleware, so any future re-run measures the provider honestly. Re-opening OpenAI for generation is a product decision (accept the quality drop for the cost win) or a newer-model re-run — the harness makes that a one-command comparison. A same-day round-2 sweep over the upper tiers confirmed the picture: gpt-4o (97% ai-mode, judge 60.14, $0.0114) is dominated by gpt-4o-mini; gpt-4.1 (80%, 69.50, $0.0093) is dominated by Haiku on all three axes; and Sonnet 4.6 free-JSON (93%, 76.93, $0.0236) beats Haiku on both reliability and quality at 3.4× cost — an in-posture quality tier a tenant can already select via `ai.surfaceModels` without any code change. A round-3 sweep over the GPT-5.4 reasoning family found the strongest OpenAI candidate so far: gpt-5.4-mini (90% ai-mode, judge 66.07 — 0.2 pts under the exact −10% cutoff, half of Haiku's cost, fastest model tested at 2.4s); gpt-5.4-nano fails (70%, 57.67). A round-4 confirmation (same-sweep, 16 prompts including 6 new production-shaped complex cases, 5 samples, N=80/model) showed the round-3 judge gap was mostly sampling noise: gpt-5.4-mini PASSES the pre-declared bar (ai-mode 86.2% vs Haiku 92.5%; judge −4.4%), with one counter-signal — Haiku went 100% on the complex cases vs gpt-5.4-mini's 80%. Enabling OpenAI for generation is now a live product decision with the verification bar met; the Pareto frontier across all nine free-JSON model runs is Sonnet (quality) / Haiku (balance + complex-case reliability) / gpt-5.4-mini (speed + cost at bar-passing quality) / gpt-4o-mini (minimum cost).

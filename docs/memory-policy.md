...
| `memory.allowedKinds` | csv | `""` (empty = none) | closed-enum: `recovery_rationale,run_summary,runbook_fragment,patch_rationale,generated_workflow,workflow_vector` | Per-kind opt-in. Empty CSV with `memory.enabled=true` is a valid "memory feature on but no kinds active yet" state. |

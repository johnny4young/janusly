...
export const GENERATE_WORKFLOW_SYSTEM_PROMPT = [
  ...
  "EXAMPLE — business-hours prompt (\"when a webhook arrives, post it to Slack channel credential ops_alerts, but only outside working hours Mon-Fri 09:00-17:00 Europe/Madrid\"). The window check is a read-side tool node and the branch rides an edge condition on `inWindow`:",
  '{"dslVersion":"1.0","id":"after_hours_alerts","name":"After-hours alerts","nodes":[{"id":"on_webhook","type":"webhook_received","config":{"endpointKey":"alerts"}},{"id":"check_hours","type":"tool","config":{"tool":"time.window","input":{"timeZone":"Europe/Madrid","windows":[{"days":[1,2,3,4,5],"start":"09:00","end":"17:00"}]}}},{"id":"notify_oncall","type":"tool","config":{"tool":"slack.post","input":{"credential":"ops_alerts","text":"After-hours alert received."}}},{"id":"hold_for_morning","type":"noop","config":{}}],"edges":[{"from":"on_webhook","to":"check_hours"},{"from":"check_hours","to":"notify_oncall","condition":"context.check_hours.output.result.inWindow === false"},{"from":"check_hours","to":"hold_for_morning","condition":"context.check_hours.output.result.inWindow === true"}]}',

...
export const GENERATE_WORKFLOW_SYSTEM_PROMPT = [
  ...
  "Examples: `to: alice@example.com subject: invoice text: thanks` → `input: { to: 'alice@example.com', subject: 'invoice', text: 'thanks' }` for email.send; `credential: slack_ops text: deploy started` → `input: { credential: 'slack_ops', text: 'deploy started' }` for slack.post; `credential: hooks url: https://hooks.example.com/x payload: {event:'deploy'}` → `input: { credential: 'hooks', url: 'https://hooks.example.com/x', payload: { event: 'deploy' } }` for webhook.send; `url: https://api.example.com/users` → `input: { url: 'https://api.example.com/users' }` for http.request; `credential: customer_db sql: select id from customers where status = $1 params: ['active']` → `input: { credential: 'customer_db', sql: 'select id from customers where status = $1', params: ['active'] }` for db.query.read.",

...
export const GENERATE_WORKFLOW_SYSTEM_PROMPT = [
  ...
  "SUPPORTED AI-GENERATION TYPES: only emit nodes of these 13 types: 'noop', 'http', 'transform', 'condition', 'ai', 'tool', 'agent', 'router', 'approval', 'human_form', 'loop', 'parallel_fork', 'join'. The platform supports more runtime types outside this direct-emission subset. For those, emit 'noop' placeholders named after the requested step: Pass 2 auto-promotes the wired families (wait_until via wait/sleep/pause/delay ids, schedule via schedule/cron/every/daily/weekly/monthly/hourly ids, and exposed MCP tools via mcp_<connectionAlias>_<toolName> when the MCP section below is present); the remaining operator-wired types (multi_agent, webhook, webhook_received, agent_reflection, router_llm, subworkflow, email_received, file_dropped, mcp_server_event) are added by the operator in the Inspector after generation. Examples: id='crew_review' type='noop', id='wait_24h' type='noop', id='ext_webhook' type='noop', id='schedule_daily' type='noop', id='on_webhook_received' type='noop', id='on_email_received' type='noop', id='on_file_dropped' type='noop'. Use 'agent' for a single autonomous step; use noop placeholders for teams/crews/groups that need multi_agent promotion. Use 'approval' when the prompt asks for a yes/no human gate. Use 'human_form' when the prompt asks a person to provide structured data, fill a request, confirm fields, or complete a review form. Use 'loop' for batch / for-each iterations.",
  ...
  "Operator-only node types are not valid AI-generation output. Use noop placeholders for multi_agent, webhook, webhook_received, wait_until, subworkflow, router_llm, agent_reflection, schedule, email_received, file_dropped, mcp_server_event, and exposed MCP-tool requests; Pass 2 promotes only the wired placeholder families, and the Inspector handles the rest.",
  "TRIGGER-INTENT NAMING: when a noop placeholder represents an event-driven trigger that STARTS the workflow (the user prompt mentioned 'when a JSON event arrives', 'when an email arrives at …', 'on a new file in the bucket', 'when an MCP resource changes', etc.), give it a descriptive id (e.g. id='on_webhook_received', id='on_email_received', id='on_file_dropped', id='on_mcp_event') and leave `config` empty `{}`. The operator promotes it to a real webhook_received / email_received / file_dropped / mcp_server_event trigger node in the Inspector — there is no auto-promotion for triggers because each needs an operator-supplied endpoint key, alias, bucket, or connection.",

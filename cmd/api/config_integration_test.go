...
func TestExecutableRejectsInvalidConfigurationBeforeDatabase(t *testing.T) {
	...
	for _, environment := range []string{"development", "production"} {
		for _, key := range []string{"JANUSLY_PORT", "JANUSLY_INTERNAL_HOST", "JANUSLY_REAPER_INTERVAL_MS", "JANUSLY_REAPER_THRESHOLD_MS", "JANUSLY_REAPER_THRESHOLD_FLOOR_MS", "JANUSLY_STALLED_NODE_THRESHOLD_MINUTES"} {
